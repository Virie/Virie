...
ethash_light_t ethash_light_filecached(uint64_t block_number, const char* dir_name)
{
	...
	{
		ETHASH_CRITICAL("snprintf failed at " FILE_LINE_LOCATION);
		return NULL;
	}
	...
	if (!fullname)
	{
		ETHASH_CRITICAL("Could not create the light cache pathname at " FILE_LINE_LOCATION);
		return NULL;
	}
	...
	if (f)
	{
		...
		{
			ETHASH_CRITICAL("Could not query size of file: \"%s\" at " FILE_LINE_LOCATION, fullname);
			fclose(f);
			free(fullname);
			return NULL;
		}
		if (found_size != size)
		{
			ETHASH_CRITICAL("Incorrect file size: %u, expected: %u for \"%s\" at " FILE_LINE_LOCATION, found_size, size, fullname);
			fclose(f);
			free(fullname);
			return NULL;
		}
		...
		if (!ret)
		{
			ETHASH_CRITICAL("calloc failed at " FILE_LINE_LOCATION);
			fclose(f);
			free(fullname);
			return NULL;
		}
		...
		if (!ret->cache)
		{
			ETHASH_CRITICAL("malloc failed at " FILE_LINE_LOCATION);
			free(ret);
			fclose(f);
			free(fullname);
			return NULL;
		...
		if (bytes_read != size)
		{
			ETHASH_CRITICAL("fread failed: %u bytes read, while %u was expected, at " FILE_LINE_LOCATION, bytes_read, size);
			free(ret->cache);
			free(ret);
			fclose(f);
			free(fullname);
	...
	if (f)
	{
		...
		{
			ETHASH_CRITICAL("fwrite failed: %u bytes written while %u was expected at " FILE_LINE_LOCATION, bytes_written, size);
			ret = NULL;
		}
		fclose(f);
	}
	...
	{
		ETHASH_CRITICAL("cannot open file %s for writing, at " FILE_LINE_LOCATION, fullname);
		ret = NULL;
	}
