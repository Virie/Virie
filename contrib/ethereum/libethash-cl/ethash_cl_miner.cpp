...
string ethash_cl_miner::platform_version(unsigned _platformId)
{
  vector<cl::Platform> platforms = getPlatforms();
  if (platforms.empty())
  {
	...
	return searchForAllDevices(_platformId, [&requiredSize](cl::Device const& _device) -> bool
		{
			...
			if (result >= requiredSize)
			{
				ETHCL_LOG(
					...
					<< "] with " << result << " bytes of GPU memory, while " << requiredSize << " bytes of memory required"
				);
				return true;
			}
...
void ethash_cl_miner::listPlatforms()
{
  std::stringstream ss;
  vector<cl::Platform> platforms = getPlatforms();
  size_t i = 0;
  for (auto& p : platforms)
    ss << "[" << i++ << "] " << p.getInfo<CL_PLATFORM_NAME>() << std::endl;
  ETHCL_LOG("platforms:" << std::endl << ss.str());
}
...
void ethash_cl_miner::listAllDevices()
{
  std::stringstream ss;
  vector<cl::Platform> platforms = getPlatforms();
  for(size_t pid = 0; pid < platforms.size(); ++pid)
  {
    ...
    ss << "PLATFORM [" << pid << "]: " << platform.getInfo<CL_PLATFORM_NAME>() << std::endl;
	  vector<cl::Device> devices = getDevices(platforms, static_cast<unsigned int>(pid));
	  for (size_t did = 0; did < devices.size(); ++did)
    {
      auto& device = devices[did];
      ss << std::endl;
      ss << "  device [" << did << "]: " << device.getInfo<CL_DEVICE_NAME>() << std::endl;
			ss << "    CL_DEVICE_TYPE:                 ";
      ...
      ss << std::endl;
      std::string device_ocl_vesrion("n/a");
      device.getInfo(CL_DEVICE_OPENCL_C_VERSION, &device_ocl_vesrion);
      ss << "    CL_DEVICE_OPENCL_C_VERSION:     " << device_ocl_vesrion << std::endl;
			ss << "    CL_DEVICE_GLOBAL_MEM_SIZE:      " << to_string(device.getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>()) << std::endl;
			ss << "    CL_DEVICE_MAX_MEM_ALLOC_SIZE:   " << to_string(device.getInfo<CL_DEVICE_MAX_MEM_ALLOC_SIZE>()) << std::endl;
			ss << "    CL_DEVICE_MAX_WORK_GROUP_SIZE:  " << to_string(device.getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>()) << std::endl;
    }
    
    if (devices.empty())
      ss << std::endl << "  no suitable devices" << std::endl;
    
    ss << std::endl;
  }
  ETHCL_LOG("OpenCL platforms and devices:" << std::endl << ss.str());
}
