...
DISABLE_CHANNEL_BY_DEFAULT("db", "DB accessor default channel.")
...
namespace tools
{
  namespace db
  {
    ...
    public:
      basic_db_accessor(std::shared_ptr<i_db_backend> backend, epee::shared_recursive_mutex& rwlock) : m_backend(backend), m_is_open(false), m_rwlock(rwlock)
      {}
      ~basic_db_accessor()
      {
        close();
    ...
    class transaction_region
    {
    public:
      enum class event_t { commit, abort };
      transaction_region(t_db &db, event_t event = event_t::abort, std::function<void()> on_commit = nullptr) :
        m_db(db), m_event(event), m_on_commit(std::move(on_commit))
      {
        m_was_begin = db.begin_transaction();
      }

      ~transaction_region()
      {
        m_event == event_t::commit ? commit() : abort();
      }
      ...
      void abort()
      {
        end([this] () { m_db.abort_transaction(); });
      }

      void will_commit()
      {
        m_event = event_t::commit;
      }

      void will_abort()
      {
        m_event = event_t::abort;
      }
    ...
    private:
      ...
      template <typename end_func_t>
      void end(end_func_t end_func)
      {
        if (m_was_begin)
        {
          end_func();
    ...
    /************************************************************************/
    template <typename t_db>
    class transaction_region_readonly
    {
      public:
      transaction_region_readonly(const t_db &db) : m_db(db)
      {
        m_was_begin = db.begin_readonly_transaction();
      }

      ~transaction_region_readonly()
      {
        commit();
      }
    ...
    public:
      ...
      static bool set(container_handle h, basic_db_accessor& bdb, const t_key& k, const t_value& v)
      {
        static_assert(std::is_pod<t_value>::value, "t_value must be a POD type.");
        return bdb.set_pod_object(h, k, v);
      }
      template<class t_key, class t_value>
    ...
    public:
      ...
      bool set(const t_key& k, const t_value& v)
      {
        ...
        size_cache_valid = false;
        return access_strategy_selector<is_t_access_strategy>::set(m_h, bdb, k, v);
      }
...
//      template<class t_key>
    ...
    {
    public: 
      typedef std::shared_ptr<const t_value> shared_value_type;
