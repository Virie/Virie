...
namespace currency
{
  namespace
  {
    const command_line::arg_descriptor<std::string> arg_rpc_bind_ip   = {"rpc-bind-ip", "Interface for rpc protocol", "127.0.0.1"};
    const command_line::arg_descriptor<std::string> arg_rpc_bind_port = {"rpc-bind-port", "Port for rpc protocol", std::to_string(RPC_DEFAULT_PORT)};
  }
  //-----------------------------------------------------------------------------------
...
#define CHECK_CORE_READY() if(!check_core_ready()){res.status =  CORE_RPC_STATUS_BUSY;return true;}
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool core_rpc_server::on_get_static_info(const COMMAND_RPC_GET_STATIC_INFO::request& req, COMMAND_RPC_GET_STATIC_INFO::response& res, connection_context& cntx)
  {
    return true;
  }
  //------------------------------------------------------------------------------------------------------------------------------
  bool core_rpc_server::on_get_info(const COMMAND_RPC_GET_INFO::request& req, COMMAND_RPC_GET_INFO::response& res, connection_context& cntx)
  {
    ...
    res.minimum_fee = m_core.get_blockchain_storage().get_core_runtime_config().tx_pool_min_fee(res.height);
    res.unavailable_disk_space = m_core.get_ref_stop_senders().was_event_free_space();
    //conditional values 
...
#define COPY_AVG_TO_POOL_PERF_DATA(field_name)   res.tx_pool_performance_data.field_name = pool_pd.field_name.get_avg();
    ...
    CATCH_ENTRY_CUSTOM("core_rpc_server::on_get_info", res.status = CORE_RPC_STATUS_FAILED, false)
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  bool core_rpc_server::on_scan_pos(const COMMAND_RPC_SCAN_POS::request& req, COMMAND_RPC_SCAN_POS::response& res, connection_context& cntx)
  {
    ...
    return true;
    CATCH_ENTRY_CUSTOM("core_rpc_server::on_scan_pos", res.status = CORE_RPC_STATUS_FAILED, false)
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  bool core_rpc_server::on_get_pos_mining_details(const COMMAND_RPC_GET_POS_MINING_DETAILS::request& req, COMMAND_RPC_GET_POS_MINING_DETAILS::response& res, connection_context& cntx)
  {
    ...
    res.pos_basic_difficulty = m_core.get_blockchain_storage().get_cached_next_difficulty(true).convert_to<std::string>();
    ...
    res.status = CORE_RPC_STATUS_OK;
    return true;
    CATCH_ENTRY_CUSTOM("core_rpc_server::on_get_pos_mining_details", res.status = CORE_RPC_STATUS_FAILED, false)
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  bool core_rpc_server::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res, epee::json_rpc::error& error_resp, connection_context& cntx)
  {
    ...
    params.p_custom_fill_block_template_func = nullptr;
    ...
    res.difficulty = resp.difficulty.convert_to<uint64_t>();
  ...
  bool core_rpc_server::on_get_alias_reward(const COMMAND_RPC_GET_ALIAS_REWARD::request& req, COMMAND_RPC_GET_ALIAS_REWARD::response& res, epee::json_rpc::error& error_resp, connection_context& cntx)
  {
    ...
    res.reward = get_alias_cost_from_fee(req.alias, std::max(default_tx_fee, current_median_fee));
