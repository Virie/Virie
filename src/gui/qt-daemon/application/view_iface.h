...
public:
    ...
    uint64_t unavailable_disk_space;
...
#ifdef TESTNET
      KV_SERIALIZE_EPHEMERAL_N(std::string, [](const daemon_status_info&) -> std::string {return CURRENCY_NAME_SHORT;}, "test_net")
#endif
  ...
  struct get_alias_cost_response
  {
    std::string error_code;
    uint64_t cost;
  ...
  struct get_tx_pool_info_response
  {
    std::string error_code;
