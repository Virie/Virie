...
public:
  ...
  std::string get_alias_cost(const std::string& a, uint64_t& cost);
  std::string validate_address(const std::string& addr, std::string& payment_id_str);
  ...
  std::string get_tx_pool_info(view::get_tx_pool_info_response& res);
  uint64_t get_default_fee() const;
...
private:
  ...
  view::i_view* m_pview;
  daemon_service::bc_daemon_service m_daemon_service;
