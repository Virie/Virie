...
daemon_backend::daemon_backend():m_stop_singal_sent(false),
                                 m_pview(&m_view_stub),
                                 m_rpc_proxy(new tools::core_fast_rpc_proxy(m_daemon_service.get_rpc_server())),
...
const command_line::arg_descriptor<std::string> arg_html_folder = {"html-path", "Manually set GUI html folder path", "",  command_line::flag_t::not_use_default};
const command_line::arg_descriptor<std::string> arg_xcode_stub = {"-NSDocumentRevisionsDebugMode", "Substitute for xcode bug", "",  command_line::flag_t::not_use_default};
const command_line::arg_descriptor<bool> arg_enable_gui_debug_mode = { "gui-debug-mode", "Enable debug options in GUI", false, command_line::flag_t::not_use_default };
const command_line::arg_descriptor<uint32_t> arg_qt_remote_debugging_port = { "remote-debugging-port", "Specify port for Qt remote debugging", 30333, command_line::flag_t::not_use_default };
...
bool daemon_backend::init(int argc, char* argv[], view::i_view* pview_handler)
{
  ...
  {
    int log_level = command_line::get_arg(m_vm, command_line::arg_log_level);
  ...
  auto log_dir = command_line::get_arg(m_vm, command_line::arg_log_dir);
  ...
  if (command_line::has_arg(m_vm, command_line::arg_version_libs))
    deps_version::print_deps_version(std::cout);

  if (command_line::has_arg(m_vm, command_line::arg_log_channels))
    log_space::log_singletone::enable_channels(command_line::get_arg(m_vm, command_line::arg_log_channels));
...
void daemon_backend::main_worker(const po::variables_map& m_vm)
{
  ...
  auto update_view_func = [this, &dsi] (const std::string &state) {
//    dsi.text_state = state;
    m_pview->update_daemon_status(dsi);
  };
  m_daemon_service.set_notify_func(std::move(update_view_func));

  if (!m_daemon_service.init(m_vm) || !m_daemon_service.start(false))
  {
  ...
  for (auto& wo : m_wallets)
  {
    ...
    try
    {
      ...
      tools::store_double_step(wo.second.w->get()->get_wallet_path(),
        [&wo] (const std::wstring &filename) { wo.second.w->get()->store(filename); });
    }
  ...
  m_daemon_service.stop();
  m_daemon_service.deinit();
...
//  dsi.hashrate = inf.current_network_hashrate_350;
  ...
  dsi.unavailable_disk_space = inf.unavailable_disk_space ? 1 : 0;
...
void daemon_backend::init_wallet_entry(wallet_vs_options& wo, uint64_t id)
{
  ...
  wo.core_conf = m_daemon_service.get_currency_core().get_blockchain_storage().get_core_runtime_config();
}
std::string daemon_backend::get_tx_pool_info(view::get_tx_pool_info_response& res)
{
  std::list<currency::extra_alias_entry> al_list;
  m_daemon_service.get_currency_core().get_tx_pool().get_aliases_from_tx_pool(al_list);
...
uint64_t daemon_backend::get_default_fee() const
{
...
{
  currency::blockchain_storage& bcs = m_daemon_service.get_currency_core().get_blockchain_storage();

  m_daemon_service.get_offers_service().get_offers_by_id(hashes, offers);
...
std::string daemon_backend::get_my_offers(const bc_services::core_offers_filter& filter, std::list<bc_services::offer_details_ex>& offers)
{
  ...
  bc_services::filter_offers_list(offers, filter, m_daemon_service.get_currency_core().get_blockchain_storage().get_core_runtime_config().get_core_time());
...
std::string daemon_backend::open_wallet(const std::wstring& path, const std::string& password, view::open_wallet_response& owr)
{
  ...
  w->set_core_proxy(std::shared_ptr<tools::i_core_proxy>(new tools::core_fast_rpc_proxy(m_daemon_service.get_rpc_server())));
...
std::string daemon_backend::generate_wallet(const std::wstring& path, const std::string& password, view::open_wallet_response& owr)
{
  ...
  {
    return API_RETURN_CODE_FAIL + std::string(":failed to save file");
  }
  wallet_vs_options& wo = m_wallets[owr.wallet_id];
  **wo.w = w;
  init_wallet_entry(wo, owr.wallet_id);
...
{
  m_daemon_service.get_currency_core().get_blockchain_storage().get_pos_mining_estimate(amuont_coins, time, estimate_result, pos_coins_and_pos_diff_rate, days);
  return API_RETURN_CODE_OK;
}

std::string daemon_backend::is_pos_allowed()
{
  if (m_daemon_service.get_currency_core().get_blockchain_storage().is_pos_allowed())
...
{
  if (pevents_handler != nullptr)
    m_daemon_service.get_currency_core().get_blockchain_storage().add_event_handler("GUI", pevents_handler);
  else
    m_daemon_service.get_currency_core().get_blockchain_storage().del_event_handler("GUI");
}
...
std::string daemon_backend::close_wallet(size_t wallet_id)
{
  ...
  try
  {
    ...
    tools::store_double_step(it->second.w->get()->get_wallet_path(),
      [it] (const std::wstring &filename) { it->second.w->get()->store(filename); });
    m_wallets.erase(it);
  }

  catch (const std::exception& e)
  {
    return API_RETURN_CODE_FAIL + std::string(":failed to save file when close");;
  }
  //m_pview->hide_wallet();
  return API_RETURN_CODE_OK;
}
...
{
  if (m_daemon_service.get_currency_core().get_blockchain_storage().get_aliases_count() > API_MAX_ALIASES_COUNT)
...
std::string daemon_backend::get_alias_info_by_address(const std::string& addr, currency::alias_rpc_details& res_details)
{
  ...
  if (!currency::get_account_address_from_str(apa, addr))
    return API_RETURN_CODE_BAD_ARG;
  std::string alias = m_daemon_service.get_currency_core().get_blockchain_storage().get_alias_by_address(apa);
  if (!alias.size())
    return API_RETURN_CODE_FILE_NOT_FOUND;
  return get_alias_info_by_name(alias, res_details);
}
...
std::string daemon_backend::get_alias_info_by_name(const std::string& name, currency::alias_rpc_details& res_details)
{
  ...
  currency::extra_alias_entry eaeb = AUTO_VAL_INIT(eaeb);
  if (!m_daemon_service.get_currency_core().get_blockchain_storage().get_alias_info(name, eaeb))
  {
    LOG_PRINT_L0("Not found alias: " << name);
    return API_RETURN_CODE_FILE_NOT_FOUND;
  }

  eaeb.m_alias = name;
  if (!currency::alias_info_to_rpc_alias_info(eaeb, res_details))
    return API_RETURN_CODE_FAIL;
  return API_RETURN_CODE_OK;
}

std::string daemon_backend::get_alias_cost(const std::string& a, uint64_t& cost)
{
  ...
  cost = rsp.reward + rsp.reward/10; //add 10% of price to be sure
  return rsp.status;
...
//    return API_RETURN_CODE_BAD_ARG_WRONG_FEE;
  ...
  if (!payment_id.from_hex(boost::trim_copy(tp.payment_id)))
    return API_RETURN_CODE_BAD_ARG_WRONG_PAYMENT_ID; // payment id must be in hex-format

  for(const auto& dest: tp.destinations)
  {
    dsts.push_back(currency::tx_destination_entry());
    dsts.back().addr.resize(1);
    currency::bc_payment_id embedded_payment_id;
    if (!tools::get_transfer_address(dest.address, dsts.back().addr.back(), embedded_payment_id, m_rpc_proxy.get()))
    {
      return API_RETURN_CODE_BAD_ARG_INVALID_ADDRESS;
    }
    if(!currency::parse_amount(dsts.back().amount, dest.amount))
    {
      return API_RETURN_CODE_BAD_ARG_WRONG_AMOUNT;
    }
    if (!embedded_payment_id.empty())
    {
      if (!payment_id.empty() && embedded_payment_id != payment_id)
...
std::string daemon_backend::accept_proposal(size_t wallet_id, const crypto::hash& contract_id)
{
  ...
  try
  {
    w.w->get()->accept_proposal(contract_id, get_default_fee());
    //TODO: add some 
    return API_RETURN_CODE_OK;
  }
  catch (...)
...
std::string daemon_backend::backup_wallet(uint64_t wallet_id, const std::wstring& path)
{
  ...
  try
  {
    tools::store_double_step(path, [&w] (const std::wstring &filename) { w.w->get()->store(filename); });
  }
  catch (...)
  {
    return API_RETURN_CODE_FAIL + std::string(":failed to save file");
...
void daemon_backend::prepare_wallet_status_info(wallet_vs_options& wo, view::wallet_status_info& wsi)
{
  ...
  wsi.balance = wo.w->get()->balance(wsi.unlocked_balance, wsi.awaiting_in, wsi.awaiting_out, wsi.mined_total);
}
...
std::string daemon_backend::get_offers_ex(const bc_services::core_offers_filter& cof, std::list<bc_services::offer_details_ex>& offers, uint64_t& total_count)
{
  ...
  m_daemon_service.get_offers_service().get_offers_ex(cof, offers, total_count, m_daemon_service.get_currency_core().get_blockchain_storage().get_core_runtime_config().get_core_time());
  return API_RETURN_CODE_OK;
}


std::string daemon_backend::validate_address(const std::string& addr_str, std::string& payment_id_str)
{
  currency::account_public_address acc = AUTO_VAL_INIT(acc);
  currency::bc_payment_id payment_id = AUTO_VAL_INIT(payment_id);
  if (currency::get_account_address_and_payment_id_from_str(acc, payment_id, addr_str))
  {
    if (!payment_id.empty())
      payment_id_str = payment_id.get_hex();
    return API_RETURN_CODE_TRUE;
  }
  else
    return API_RETURN_CODE_FALSE;
...
{
  log_space::log_singletone::set_thread_log_prefix("[WALLET:" + w->get()->get_account().get_public_address_str().substr(0, 8) + "]");
  LOG_PRINT_GREEN("Wallet handler thread started, addr: " << w->get()->get_account().get_public_address_str(), LOG_LEVEL_0);
  ...
  need_to_update_wallet_info = false;
  while (!w.unlocked_get()->is_stopped())
  {
    try
    {
      if (need_to_update_wallet_info)
      {
        view::wallet_status_info wsi = AUTO_VAL_INIT(wsi);
        prepare_wallet_status_info(*this, wsi);
        wsi.wallet_state = this->wallet_state;
