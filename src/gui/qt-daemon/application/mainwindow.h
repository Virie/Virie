...
//  QString get_all_offers(const QString& param);
  ...
  QString get_alias_cost(const QString& param);
...
private:
  ...
  void on_core_event(const std::string& event_name, const currency::core_event_v& e) override;
  void on_complete_events() override;
  void on_clear_events() override;

  //------- i_view ---------
  bool update_daemon_status(const view::daemon_status_info& info) override;
  bool on_backend_stopped() override;
  bool show_msg_box(const std::string& message) override;
  bool update_wallet_status(const view::wallet_status_info& wsi) override;
  bool update_wallets_info(const view::wallets_summary_info& wsi) override;
  bool money_transfer(const view::transfer_event_info& tei) override;
  bool wallet_sync_progress(const view::wallet_sync_progres_param& p) override;
  bool money_transfer_cancel(const view::transfer_event_info& wsi) override;
  bool init(const std::string& path) override;
  bool pos_block_found(const currency::block& block_found) override;
  bool set_options(const view::gui_options& opt) override;
  ...
  void apply_default_window_size();
  ...
  std::list<currency::core_event> m_events;
  critical_section m_events_lock;
...
BOOST_CLASS_VERSION(MainWindow::app_config, 1)
