...
namespace nodetool
{
  ...
  struct i_p2p_endpoint
  {
    ...
    virtual std::list<std::pair<peerid_type, t_connection_context>> get_connections_history() = 0;
    virtual bool block_address(uint32_t ip, uint32_t port) = 0;
    virtual bool address_blocked(uint32_t ip, uint32_t port) = 0;
    virtual bool address_allowed_to_outgoing(uint32_t ip, uint32_t port) = 0;
    virtual bool add_address_fail(uint32_t ip, uint32_t port) = 0;
  ...
  struct p2p_endpoint_stub: public i_p2p_endpoint<t_connection_context>
  {
    ...
    virtual std::list<std::pair<peerid_type, t_connection_context>> get_connections_history()
    {
      return {};
    }
