...
namespace nodetool
{
  const static boost::uuids::uuid P2P_NETWORK_ID = { { 0x11, 0x10, 0x01, 0x11, 0x01, 0x02, 0x01, 0x01, 0x10, 0x11, P2P_NETWORK_ID_BYTE_A, 0x10, 0x11, 0x11, 0x01, P2P_NETWORK_ID_VER} };
  ...
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  std::list<std::pair<peerid_type, typename t_payload_net_handler::connection_context>> node_server<t_payload_net_handler>::get_connections_history()
  {
    return m_connections_history.template get_list<typename t_payload_net_handler::connection_context>();
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::is_address_allowed(uint32_t ip, uint32_t port)
  {
    if (m_offline_mode)
      return false;

    CRITICAL_REGION_LOCAL(m_blocked_ips_lock);
    auto address = net_address { ip, port };
    auto it = m_blocked_ips.find(address);
    if(it == m_blocked_ips.end())
      return true;
    if (time(nullptr) - it->second > P2P_IP_BLOCKTIME )
    {
      m_blocked_ips.erase(it);
      m_allowed_blocked_ips.erase(address);
      LOG_PRINT_CYAN("Ip " << string_tools::get_ip_string_from_int32(ip) << "is unblocked.", LOG_LEVEL_0);
      return true;
    }
    if (m_allowed_blocked_ips.count(address))
      return true;
    if (m_allowed_blocked_ips.size() < P2P_ALLOWED_BLOCKLIST_MAX_SIZE)
    {
      m_allowed_blocked_ips.insert(address);
      return true;
    }
    return false;
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::block_address(uint32_t ip, uint32_t port)
  {
    auto address = net_address { ip, port };
    CRITICAL_REGION_LOCAL(m_blocked_ips_lock);
    m_blocked_ips[address] = time(nullptr);
    LOG_PRINT_CYAN("Ip " << string_tools::get_ip_string_from_int32(address.ip) << " blocked.", LOG_LEVEL_0);
    return true;
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::address_blocked(uint32_t ip, uint32_t port)
  {
    auto address = net_address { ip, port };
    CRITICAL_REGION_LOCAL(m_blocked_ips_lock);
    return m_blocked_ips.find(address) != m_blocked_ips.end();
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::address_allowed_to_outgoing(uint32_t ip, uint32_t port)
  {
    auto address = net_address { ip, port };
    CRITICAL_REGION_LOCAL(m_blocked_ips_lock);
    return m_allowed_blocked_ips.find(address) != m_allowed_blocked_ips.end();
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::add_address_fail(uint32_t ip, uint32_t port)
  {
    auto address = net_address { ip, port };
  ...
  //-----------------------------------------------------------------------------------
  inline nodetool::net_address get_peer_from_string(const std::string& node_addr)
  {
    nodetool::net_address pe = AUTO_VAL_INIT(pe);
    if (!string_tools::parse_peer_from_string(pe.ip, pe.port, node_addr))
      throw std::invalid_argument(node_addr);
    return pe;
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::handle_command_line(const boost::program_options::variables_map& vm)
  {
    ...
    m_use_only_priority_peers = command_line::has_arg(vm, arg_p2p_use_only_priority_nodes);
    m_hide_my_port = command_line::has_arg(vm, arg_p2p_hide_my_port);
    ...
    {
      const std::vector<std::string>& perrs = command_line::get_arg(vm, arg_p2p_add_peer);
      for(const std::string& pr_str: perrs)
      {
        nodetool::peerlist_entry pe = AUTO_VAL_INIT(pe);
        pe.id = crypto::rand<uint64_t>();
        pe.adr = get_peer_from_string(pr_str);
    ...
    {
      const std::vector<std::string>& perrs = command_line::get_arg(vm, arg_p2p_add_priority_node);
      for(const std::string& pr_str: perrs)
        m_priority_peers.push_back(get_peer_from_string(pr_str));
    }

    if (command_line::has_arg(vm, arg_p2p_seed_node))
    {
      const std::vector<std::string>& seed_perrs = command_line::get_arg(vm, arg_p2p_seed_node);
      m_seed_nodes.reserve(seed_perrs.size());
      for(const std::string& pr_str: seed_perrs)
        m_seed_nodes.push_back(get_peer_from_string(pr_str));
    }
    CATCH_ENTRY_L0("node_server::handle_command_line", false)
  ...
  {
    template<typename T>
    bool append_net_address(T& nodes, const std::string& host, const std::string& port)
    {
  ...
  #define ADD_HARDCODED_SEED_NODE(addr, port) append_net_address(m_seed_nodes, std::string(addr), std::to_string(port));
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
...
#ifndef TESTNET
    ADD_HARDCODED_SEED_NODE("144.217.70.124", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("144.217.74.224", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("45.125.192.66", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("45.125.195.226", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("148.251.138.12", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("136.243.64.159", P2P_DEFAULT_PORT);
    ADD_HARDCODED_SEED_NODE("136.243.41.89", P2P_DEFAULT_PORT);
#endif

#ifdef P2P_START_SEEDS
    {
      const std::string addrs{P2P_START_SEEDS};
      for(std::string::size_type pos_start=0, pos_end, len = addrs.length(); pos_start < len; pos_start = pos_end + 1)
      {
        pos_end = addrs.find(',', pos_start);
        if (std::string::npos == pos_end)
          pos_end = len;
        ADD_HARDCODED_SEED_NODE(addrs.substr(pos_start, pos_end - pos_start), P2P_DEFAULT_PORT);
      }
    }
#endif
  ...
  bool node_server<t_payload_net_handler>::run(bool sync_call)
  {
    ...
    m_net_server.add_idle_handler(boost::bind(&node_server<t_payload_net_handler>::idle_worker, this), 1000, "node::server::idle_worker");
    m_net_server.add_idle_handler(boost::bind(&t_payload_net_handler::on_idle, &m_payload_handler), 1000, "t_payload_net_handler::on_idle");
  ...
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  std::string node_server<t_payload_net_handler>::print_blocklist()
  {
    std::stringstream ss;
    ss << (m_blocked_ips.empty() ? "Blocked ips list is empty" : "Blocked ips:\n");
    for (auto &it : m_blocked_ips)
    {
      auto ip = boost::asio::detail::socket_ops::network_to_host_long(it.first.ip);
      ss << boost::asio::ip::address_v4(ip).to_string() << " " << it.first.port << std::endl;
    }
    return ss.str();
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  void node_server<t_payload_net_handler>::clear_blocklist()
  {
    m_blocked_ips.clear();
    m_allowed_blocked_ips.clear();
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  ...
  bool node_server<t_payload_net_handler>::do_handshake_with_peer(peerid_type& pi, p2p_connection_context& context_, bool just_take_peerlist)
  {
      ...
      [this, &pi, &ev, &hsh_result, &just_take_peerlist](int code, const typename COMMAND_HANDSHAKE::response& rsp, p2p_connection_context& context)
    {
      ...
      if(!handle_remote_peerlist(rsp.local_peerlist, rsp.node_data.local_time, context))
      {
        ...
        add_address_fail(context.m_remote_ip, context.m_remote_port);
        return;
      }
      hsh_result = true;
  ...
  bool node_server<t_payload_net_handler>::is_peer_id_used(const peerid_type id)
  {
    ...
    bool used = false;
    m_net_server.get_config_object().foreach_connection([&used, id](const p2p_connection_context& cntxt)
    {
      if (id == cntxt.peer_id)
      {
        used = true;
...
//-----------------------------------------------------------------------------------
  ...
  bool node_server<t_payload_net_handler>::is_peer_used(const peerlist_entry& peer)
  {
    ...
    bool used = false;
    m_net_server.get_config_object().foreach_connection([&used, &peer](const p2p_connection_context& cntxt)
    {
  ...
  {
    LOG_PRINT_L0("Connecting to " << na << " (white=" << white << ", last_seen: " << (last_seen_stamp?misc_utils::get_time_interval_string(time(NULL) - last_seen_stamp):"never" ) << ")...");
  ...
  bool node_server<t_payload_net_handler>::make_new_connection_from_peerlist(bool use_white_list)
  {
    ...
    while (rand_count < (max_random_index + 1) * 3 && try_count < 10 && !m_net_server.is_stop_signal_sent() && peer_index < peers_count)
    {
      ...
      if (!is_address_allowed(pe.adr.ip, pe.adr.port))
      {
        ++peer_index;
        continue;
      }
      ...
      LOG_PRINT_L1("Selected peer: " << pe.id << " " << pe.adr << " [white=" << use_white_list << "] last_seen: " << (pe.last_seen ? misc_utils::get_time_interval_string(time(NULL) - pe.last_seen) : "never"));
  ...
  bool node_server<t_payload_net_handler>::connections_maker()
  {
    ...
    for(const net_address& na: m_priority_peers)
    {
      ...
      {
        LOG_PRINT_L0("connection to priority node " << na << " failed");
      }
    }
 ...
 //-----------------------------------------------------------------------------------
  ...
  bool node_server<t_payload_net_handler>::get_local_node_data(basic_node_data& node_data)
  {
    ...
    if(!m_hide_my_port)
      node_data.my_port = get_external_peer_port();
    else 
      node_data.my_port = 0;
  ...
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::log_seeds(std::ostream &s)
  {
    std::stringstream ss;
    for(const auto & seed:m_seed_nodes)
    {
      ss << string_tools::get_ip_string_from_int32(seed.ip) << ":" << seed.port << ENDL;
    }
    s << ENDL << "Seed nodes:" << ENDL << ss.str() << ENDL;
    return true;
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::log_peerlist(std::ostream &s)
  {
    std::list<peerlist_entry> pl_wite;
    std::list<peerlist_entry> pl_gray;
    m_peerlist.get_peerlist_full(pl_gray, pl_wite);
    s << ENDL << "Peerlist white:" << ENDL << print_peerlist_to_string(pl_wite) << ENDL << "Peerlist gray:" << ENDL << print_peerlist_to_string(pl_gray);
    return true;
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  ...
  {
    m_connections_history.add(context);
    LOG_PRINT_L2("["<< net_utils::print_connection_context(context) << "] CLOSE CONNECTION");
  }
  //-----------------------------------------------------------------------------------
  template<class t_payload_net_handler>
  bool node_server<t_payload_net_handler>::log_maintainers(std::ostream &s)
  {
    std::stringstream ss;
    ss << "Maintainers info:" << std::endl
       << "\ttimestamp: " << m_maintainers_info_local.timestamp << std::endl
       << "\tversion: " << static_cast<size_t>(m_maintainers_info_local.ver_major) << "." << static_cast<size_t>(m_maintainers_info_local.ver_minor)
       << " rev: " << static_cast<size_t>(m_maintainers_info_local.ver_revision) << " build: " << m_maintainers_info_local.build_no << std::endl
       << "\tconditions:" << std::endl;
    for(const auto& cond : m_maintainers_info_local.conditions)
    {
      ss << "\t\t alert mode: " << static_cast<size_t>(cond.alert_mode) << "\tif build less then: " << cond.if_build_less_then << std::endl;
    }
    s << ss.str();
    return true;
  }
