...
#pragma pack(pop)
  ...
  inline std::ostream& operator<<(std::ostream& o, const net_address na)
  {
    o << epee::string_tools::get_ip_string_from_int32(na.ip) << ":" << boost::lexical_cast<std::string>(na.port);
    return o;
  }
  ...
  std::string print_peerlist_to_string(const std::list<peerlist_entry>& pl)
  {
    ...
    {
      ss << std::setfill ('0') << std::setw (16) << std::hex << std::noshowbase
         << pe.id << "\t" << pe.adr
...
#define MAX_CONNECTIONS_HISTORY_SIZE 20
