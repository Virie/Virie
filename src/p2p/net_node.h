...
#include "connections_history.h"
using namespace epee;

#undef LOG_DEFAULT_CHANNEL 
#define LOG_DEFAULT_CHANNEL "p2p"
DISABLE_CHANNEL_BY_DEFAULT("p2p", "Peer to peer server default channel.")
...
namespace nodetool
{
  ...
  public:
    ...
    node_server(t_payload_net_handler& payload_handler):m_allow_local_ip(false),
                                                        ...
                                                        m_alert_mode(0)
    {
      m_maintainers_info_local.ver_major = PROJECT_VERSION_MAJ_NO;
      m_maintainers_info_local.ver_minor = PROJECT_VERSION_MIN_NO;
      m_maintainers_info_local.build_no = PROJECT_VERSION_BUILD_NO;
    }
    ...
    uint32_t get_this_peer_port() const {return m_listenning_port;}
    uint32_t get_external_peer_port() const {return m_external_port ? m_external_port : get_this_peer_port();}
    ...
    void serialize(Archive &a,  const t_version_type ver)
    {
      ...
      {
        const maintainers_entry me_temp = m_maintainers_entry_local;
        m_maintainers_entry_local = AUTO_VAL_INIT(m_maintainers_entry_local);
        handle_maintainers_entry(me_temp);
      }
    }
    // debug functions
    bool log_peerlist(std::ostream &s);
    bool log_connections();
    bool log_seeds(std::ostream &s);
    bool log_maintainers(std::ostream &s);
    ...
    std::string print_blocklist();
    void clear_blocklist();
    uint64_t get_up_time() const { return m_startup_time; }
  ...
  private:
    ...
    virtual std::list<std::pair<peerid_type, typename t_payload_net_handler::connection_context>> get_connections_history();
    virtual bool block_address(uint32_t ip, uint32_t port);
    virtual bool address_blocked(uint32_t ip, uint32_t port);
    virtual bool address_allowed_to_outgoing(uint32_t ip, uint32_t port);
    virtual bool add_address_fail(uint32_t ip, uint32_t port);
    virtual bool is_stop_signal_sent();
    //----------------- i_connection_filter  --------------------------------------------------------
    virtual bool is_address_allowed(uint32_t ip, uint32_t port);
    //-----------------------------------------------------------------------------------------------
    ...
    connections_history<p2p_connection_context, MAX_CONNECTIONS_HISTORY_SIZE> m_connections_history;
    ...
    std::map<net_address, time_t> m_blocked_ips;
    std::set<net_address> m_allowed_blocked_ips;

    critical_section m_ip_fails_score_lock;
    std::map<net_address, uint64_t> m_ip_fails_score;
  };
}
