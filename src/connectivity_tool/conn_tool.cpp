...
namespace
{
  ...
  const command_line::arg_descriptor<size_t>      arg_port               = {"port", "set port", P2P_DEFAULT_PORT};
  const command_line::arg_descriptor<size_t>      arg_rpc_port           = {"rpc_port", "set rpc port", RPC_DEFAULT_PORT};
  const command_line::arg_descriptor<uint32_t>    arg_timeout            = {"timeout", "set timeout", 2000};
  const command_line::arg_descriptor<std::string> arg_priv_key           = {"private_key", "private key to subscribe debug command", "", command_line::flag_t::not_use_default | command_line::flag_t::file_support};
  ...
  const command_line::arg_descriptor<uint64_t>    arg_log_journal_len    = { "log_journal_len", "Specify length of list of last error messages in log", 0, command_line::flag_t::not_use_default };
  ...
  const command_line::arg_descriptor<bool>        arg_get_daemon_info    = {"rpc_get_daemon_info", "request daemon state info vie rpc (--rpc_port option should be set ).", "", command_line::flag_t::not_use_default};
  const command_line::arg_descriptor<bool>        arg_get_aliases        = {"rpc_get_aliases", "request daemon aliases all list", "", command_line::flag_t::not_use_default};
  const command_line::arg_descriptor<std::string> arg_increment_build_no = { "increment_build_no", "Increment build number", "", command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<std::string> arg_update_maintainers_info = {"update_maintainers_info", "Push maintainers info into the network, update_maintainers_info=file_with_info.json", "", command_line::flag_t::not_use_default};
  const command_line::arg_descriptor<std::string> arg_update_build_no    = {"update_build_no", "Updated version number in version template file", "", command_line::flag_t::not_use_default};
  const command_line::arg_descriptor<std::string> arg_generate_genesis   = {"generate_genesis", "Generate genesis coinbase based on config file", "", command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<uint64_t>    arg_genesis_split_amount = { "genesis_split_amount", "Set split amount for generating genesis block", 0, command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<std::string> arg_get_info_flags     = { "getinfo_flags_hex", "Set of bits for rpc_get_daemon_info", "", command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<int64_t>     arg_set_peer_log_level = { "set-peer-log-level", "Set log level for remote peer", 0, command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<uint64_t>    arg_download_peer_log =  { "download-peer-log", "Download log from remote peer (starting offset)", 0, command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<std::string> arg_generate_integrated_address = { "generate-integrated-address", "Tool create integrated address from simple address and payment_id", "", command_line::flag_t::not_use_default };
  const command_line::arg_descriptor<std::string> arg_generate_restore_arrays = { "generate-restore-arrays", "Tool create arrays for restore, arg result file name", "", command_line::flag_t::not_use_default };
}
...
std::basic_string<CharT> buff_to_uint64_array(const std::basic_string<CharT>& s, const std::string& comment)
{
  using namespace std;
  ...
  hexStream << "--------- genesis.h---------\n"
    << "#pragma pack(push, 1)\n"
    << "struct genesis_tx_raw_data // " << comment << "\n"
    << "{\n"
    << "  uint64_t const v[" << count << "];\n"
    << "  uint8_t const r[" << rest_bytes << "];\n"
    << "};\n"
    << "#pragma pack(pop)\n"
    << "extern const genesis_tx_raw_data ggenesis_tx_raw;\n"
    << "\n"
    << "--------- genesis.cpp---------\n"
    << "const genesis_tx_raw_data ggenesis_tx_raw = {{ // " << comment << "\n";

  //hexStream << hex << ; //<< noshowbase;

  for (size_t i = 0, new_line = 0; i != count; i++)
  {
...
bool generate_genesis(const std::string& path_config, uint64_t premine_split_amount)
{
  ...
  const std::string add_comment = "generated: " + epee::misc_utils::get_time_str(cur_time) + ", from: " + path_config.substr(0, 20);
  ...
  {
    p.amount_vre_int = std::round(p.amount_vre_fl * COIN);
  }
  TIME_MEASURE_FINISH_MS(load_json_time);

  std::cout << "path: " << path_config << '\n'
            << "items " << gcp.payments.size() << ", load_json_time:" << load_json_time << std::endl;

  currency::block bl = boost::value_initialized<block>();

  std::vector<tx_destination_entry> destinations;
  destinations.reserve(gcp.payments.size());
  tx_destination_entry de = AUTO_VAL_INIT(de);
  de.addr.resize(1);

  uint64_t total_vre = 0;
  for (auto& p: gcp.payments)
  {

    bool r = get_account_address_from_str(de.addr.back(), p.address_vre);
    CHECK_AND_ASSERT_MES(r, false, "wrong address string: " << p.address_vre);

    de.amount = p.amount_vre_int;
    total_vre += de.amount;
  ...
  ss << '\n' << std::setw(15) << std::left << "PREMINE_AMOUNT" << std::setw(15) << std::fixed << std::left << " " << std::setw(15) << std::fixed << std::left << total_vre << "(" << print_money(total_vre) <<"VRE)\n";

  const std::string premain_str= "\n--------- currency_config.h ---------\n"
                                 "#define PREMINE_AMOUNT" +std::string(34, ' ') + std::to_string(total_vre) + "ull  // " + print_money(total_vre, true) + " VRE, " + add_comment;

  epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_magenta, true);
  epee::colored_cout::cu_cout << "GENESIS STAT: \n" << ss.str() << std::flush;
  epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_default, false);
  ...
  std::string uint64_t_array_represent = /*string_tools::*/buff_to_uint64_array(txb, add_comment);
  ...
  address.reserve(gcp.payments.size());
  uint64_t i = 0;
  for (auto& p : gcp.payments)
  {
    uint64_t key = currency::get_string_uint64_hash(p.address_vre);
    auto it = ordered_keys.find(key);
    if (it != ordered_keys.end())
    {
      LOG_ERROR("Collision found on address " << p.address_vre << " key: " << key << " offset_associated: " << it->second);
      return false;
    }
    ordered_keys[key] = i;
    address.push_back(p.address_vre.substr(0, 10));
    i++;
  }
  //dump it to file
  
  ss << "------------- genesis_acc.h -------------\n"
     << "extern const genesis_tx_dictionary_entry ggenesis_dict[" << ordered_keys.size() <<  "]; // " << add_comment << '\n'
     << '\n'
     << "-------------genesis_acc.cpp-------------\n"
     << "const std::string ggenesis_tx_pub_key_str = \"" << string_tools::pod_to_hex(get_tx_pub_key_from_extra(bl.miner_tx)) << "\"; // " << add_comment << '\n'
     ...
     << "extern const genesis_tx_dictionary_entry ggenesis_dict[" << ordered_keys.size() <<  "];\n"
     << "const genesis_tx_dictionary_entry ggenesis_dict[" << ordered_keys.size() << "] = {";

  bool comma = false;
  for (const auto & ok : ordered_keys)
  {
    ss << "\n" << (comma ? "," : "") << "{" << ok.first << "ull," << ok.second << "} // " << address[ok.second] << "...";
    comma = true;
  }
  ss << "\n};\n";
  ...
  const std::string all_info = premain_str + "\n" + uint64_t_array_represent + "\n" + ss.str();

  r = file_io_utils::save_string_to_file(path_config + ".summary.txt", all_info);
  CHECK_AND_ASSERT_MES_NO_RET(r, "failed to create " << path_config << ".summary.txt");
...
bool handle_request_stat(po::variables_map& vm, peerid_type peer_id)
{
  ...
  if(!peer_id)
  {
    ...
    {
      std::cout << "{" << ENDL << "  \"status\": \"ERROR: " << "Failed to invoke remote command COMMAND_REQUEST_PEER_ID to " << command_line::get_arg(vm, arg_ip) << ":" << command_line::get_arg(vm, arg_port) << "\"" << ENDL << "}";
      return false;
    }else
    {
      peer_id = rsp.my_id;
...
bool handle_update_maintainers_info(po::variables_map& vm)
{
  ...
  std::string path = command_line::get_arg(vm, arg_update_maintainers_info);
  ...
  if (mi.build_no == 0)
    mi.build_no = PROJECT_VERSION_BUILD_NO;
  if (mi.ver_major == 0)
    mi.ver_major = PROJECT_VERSION_MAJ_NO;
  if (mi.ver_minor == 0)
    mi.ver_minor = PROJECT_VERSION_MIN_NO;

  mi.timestamp = time(NULL);
  std::cout << "version: " << static_cast<size_t>(mi.ver_major) << "." << static_cast<size_t>(mi.ver_minor) << "." << mi.build_no
            << ", revision: " << static_cast<size_t>(mi.ver_revision)
            << ", timestamp: " << mi.timestamp << ENDL;
...
bool generate_and_print_keys()
{
  ...
  std::cout << "PUBLIC KEY: " << epee::string_tools::pod_to_hex(pk) << ENDL
    << "PRIVATE KEY: " << epee::string_tools::pod_to_hex(sk) << ENDL;
  return true;
}
//---------------------------------------------------------------------------------------------------------------
...
bool handle_download_peer_log(po::variables_map& vm)
{
  ...
  current_size = rsp.current_log_size;
  if (start_offset >= current_size)
  {
    std::cout << "ERROR: invalid start offset: " << start_offset << ", log size: " << current_size << ENDL;
    return false;
  }
  ...
  boost::filesystem::ofstream log{ epee::string_encoding::utf8_to_wstring(local_filename), boost::filesystem::ifstream::binary };
  if (!log)
  {
  ...
  while (true)
  {
    ...
    rsp = AUTO_VAL_INIT(rsp);
    req.log_chunk_offset = end_offset;
    req.log_chunk_size = std::min(chunk_size, current_size - end_offset);
    ...
    {
      std::cout << "WARNING: unpacked size: " << rsp.log_chunk.size() << ", requested: " << req.log_chunk_size << ", continued" << ENDL;
    }

    log.write(rsp.log_chunk.c_str(), rsp.log_chunk.size());

    end_offset += rsp.log_chunk.size();
...
bool handle_generate_integrated_address(po::variables_map& vm)
{
  ...
  std::string payment_id_str = add_and_payment_id.substr(off+1, add_and_payment_id.length());
  currency::bc_payment_id payment_id;
  if (!payment_id.from_hex(payment_id_str))
  {
    payment_id.id = payment_id_str;
  }

  if (address.empty() || payment_id.empty())
  {
  ...
  std::string integrated_addr = currency::get_account_address_and_payment_id_as_str(acc_addr, payment_id);
...
//---------------------------------------------------------------------------------------------------------------
bool generate_restore_arrays(po::variables_map& vm)
{
  std::string output_file_name = command_line::get_arg(vm, arg_generate_restore_arrays);

  std::string input_file_name = "MnemonicEncodingArray.txt";
  boost::system::error_code e;
  bool exists = boost::filesystem::exists(input_file_name, e);
  if (!exists)
  {
    epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_red, true);
    epee::colored_cout::cu_cout << "Error file MnemonicEncodingArray.txt doesn't exists" << std::endl;
  ...
  {
    epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_red, true);
    epee::colored_cout::cu_cout << "Error open file MnemonicEncodingArray.txt" << std::endl;
    epee::colored_cout::cu_cout.reset_color();
    return false;
  }
  ...
  std::vector<std::string> temp_words_array(words);

  for (size_t i = 0; i < temp_words_array.size() - 1; i++)  ///FIXME case size == 0
  {
    for (size_t j = 0; j < temp_words_array.size() - i - 1; j++)
    {
      if (temp_words_array.at(j) > temp_words_array.at(j + 1))
      {
        std::string temp = temp_words_array.at(j);
        temp_words_array.at(j) = temp_words_array.at(j + 1);
        temp_words_array.at(j + 1) = temp;

        int temp_index = word_indexes.at(j);
        word_indexes.at(j) = word_indexes.at(j + 1);
        word_indexes.at(j + 1) = temp_index;
      }
    }
  }
  ...
  {
    epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_red, true);
    epee::colored_cout::cu_cout << "Error open file " << output_file_name << std::endl;
    epee::colored_cout::cu_cout.reset_color();
    return false;
  }

  size_t maximum_length = 4;
  size_t in_string_numbers = 0;
  output_file << "const uint32_t words_indexes[] = {" << std::endl;
  for (size_t i = 0; i < word_indexes.size() - 1; i++) ///FIXME case size == 0
  {
    std::string print_number = std::to_string(word_indexes.at(i));
    output_file << print_number;
    if (i + 2 < word_indexes.size())
    {
      output_file << ", ";
    }

    ptrdiff_t whitspaces_needed = maximum_length - print_number.size();
    if (whitspaces_needed > 0)
    {
      output_file << std::string(whitspaces_needed, ' ');
    }

    in_string_numbers++;
    if (in_string_numbers % 25 == 0 && in_string_numbers != 0)
    {
      in_string_numbers = 0;
  ...
  maximum_length = 0;
  output_file << "const char* words_array[] = {" << std::endl;
  for (size_t i = 0; i < words.size(); i++)
  {
    size_t len = words.at(i).size();
  ...
  size_t in_string_words = 0;
  for (size_t i = 0; i < words.size(); i++)
  {
    output_file << "\"" << words.at(i) << "\"";
    if (i + 1 < words.size())
    {
      output_file << ", ";
    }
    ptrdiff_t whitspaces_needed = maximum_length - words.at(i).size();
    if (whitspaces_needed > 0)
    {
      output_file << std::string(whitspaces_needed, ' ');
    }
    in_string_words++;
    if (in_string_words % 10 == 0 && in_string_words != 0)
    {
      output_file << std::endl;
      in_string_words = 0;
    }
...
int main(int argc, char* argv_original[])
{
  ...
  argv_processor.hide_secret(arg_priv_key.name);

  string_tools::set_module_name_and_folder(argv[0]);
  log_space::get_set_log_detalisation_level(true, LOG_LEVEL_MAX);
  log_space::log_singletone::add_logger(LOGGER_CONSOLE, NULL, NULL, LOG_LEVEL_MAX);
  ...
  command_line::add_arg(desc_params, command_line::arg_log_channels);
  ...
  command_line::add_arg(desc_params, arg_update_maintainers_info);
  ...
  command_line::add_arg(desc_params, arg_generate_restore_arrays);
  ...
  else if(command_line::has_arg(vm, arg_update_maintainers_info))
  {
  ...
  else if (command_line::has_arg(vm, arg_generate_restore_arrays))
  {
    return generate_restore_arrays(vm) ? EXIT_SUCCESS : EXIT_FAILURE;
  }
  else
  {
