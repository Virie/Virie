...
#include "common/enumerate_container.h"
...
#pragma pack (pop)
  ...
  typedef i_wallet2_callback wallet2_callback_stub;
  ...
  template<typename split_strategy_t>
  struct construct_tx_param
  {
    ...
    uint8_t flags;
    split_strategy_t split_strategy;
  ...
  using construct_tx_param_t = construct_tx_param<std::function<decltype(detail::void_split_strategy)>>;
  ...
  class wallet2
  {
    wallet2(const wallet2&) : m_watch_only(false),
                              m_do_rise_transfer(false),
                              m_stop(false),
                              m_wcallback(new wallet2_callback_stub())
    {}
  public:
    wallet2() : m_watch_only(false),
                m_do_rise_transfer(false),
...
#define   WALLET_TRANSFER_DETAIL_FLAG_COLD_SIG_RESERVATION             uint32_t(1 << 4) // transfer is reserved for cold-signing (unsigned tx was created and passed for signing)
    ...
    struct transfer_details_base
    {
      ...
      bool is_reserved_for_cold_sign() const { return ( (m_flags & WALLET_TRANSFER_DETAIL_FLAG_COLD_SIG_RESERVATION) != 0 ); }
      bool is_blocked() const { return ( (m_flags & WALLET_TRANSFER_DETAIL_FLAG_BLOCKED) != 0 ); }
    ...
    typedef std::unordered_multimap<currency::bc_payment_id, payment_details> payment_container;
    ...
    struct percent_calculator
    {
      template<typename cb_t>
      void calculate(uint64_t current_height, uint64_t max_height, cb_t cb)
      {
        if (max_height <= m_start_height)
          return;
        uint64_t next_percent = (100 * (current_height - m_start_height)) / (max_height - m_start_height);
      ...
      void set_start_height(uint64_t start_height)
      {
        m_last_percent = 0;
    ...
    private:
      ...
      uint64_t m_last_percent;
    };

    void assign_account(const currency::account_base& acc);
    void generate(const std::wstring& wallet, const std::string& password);
    wallet2 clone(bool watch_only) const;
    void restore(const std::wstring& path, const std::string& pass, const std::string& restore_data, bool watch_only = false);
    void load(const std::wstring& wallet, const std::string& password);
    void store();
    void store(const std::wstring& path);
    void store_watch_only(const std::wstring& path, const std::string& password);
    ...
    bool is_stopped() const { return m_stop.load(std::memory_order_relaxed); }
    void reset_creation_time(uint64_t timestamp);

    //i_wallet2_callback* callback() const { return m_wcallback; }
    //void callback(i_wallet2_callback* callback) { m_callback = callback; }
    void callback(std::shared_ptr<i_wallet2_callback> callback)
    {
      m_do_rise_transfer = false;
      m_wcallback = std::move(callback);
      if (!m_wcallback)
        m_wcallback.reset(new wallet2_callback_stub());
      else
        m_do_rise_transfer = true;
    }
    ...
    bool refresh_noexept(size_t& blocks_fetched);
    ...
    uint64_t balance(uint64_t& unlocked, uint64_t& awaiting_in, uint64_t& awaiting_out, uint64_t& mined) const;
    uint64_t balance(uint64_t& unlocked) const;
    ...
    void transfer(const std::vector<currency::tx_destination_entry>& dsts,
      ...
      split_strategy_t destination_split_strategy,
      ...
      std::string *p_signed_tx_blob_str = nullptr)
    {
      ...
      ctp.flags = flags;
      ctp.split_strategy = destination_split_strategy;
      ...
      if (m_watch_only)
      {
        std::string reserve_path = "virie_tx_unsigned";
        boost::filesystem::path pp = boost::filesystem::path(m_wallet_file).parent_path();
        if (!pp.empty())
          reserve_path = (pp / reserve_path).string();
        bool r = store_unsigned_tx_to_file_and_reserve_transfers(ftp, p_signed_tx_blob_str ? "" : reserve_path, p_signed_tx_blob_str);
        THROW_IF_FALSE_WALLET_INT_ERR_EX(r, "failed to store unsigned tx");
        LOG_PRINT_GREEN("[wallet::transfer]" << " prepare_transaction_time: " << print_fixed_decimal_point(prepare_transaction_time, 3), LOG_LEVEL_0);
        return;
      }
      ...
      LOG_PRINT_GREEN("[wallet::transfer]"
    ...
    void get_payments(const currency::bc_payment_id& payment_id, std::list<payment_details>& payments, uint64_t min_height = 0) const;

    template<typename callback_t>
    void enumerate_transfers_history(callback_t cb, bool forward = true) const;
    template<typename callback_t>
    ...
    bool get_transfer_address(const std::string& adr_str, currency::account_public_address& addr, currency::bc_payment_id& payment_id);
    ...
    inline void serialize(t_archive &a, const unsigned int ver)
    {
      ...
      if (ver > WALLET_LAST_VERSION_WITHOUT_COLD_SIGN)
      {
        a & m_pending_key_images;
      }
      if (ver > WALLET_LAST_VERSION_WITHOUT_COLD_SIGN_RESERVES)
      {
        a & m_cold_sign_reserves;
      }
    }
    ...
    bool try_mint_pos(const currency::account_public_address &miner_address);
    bool try_mint_pos();
    //for unit tests
    ...
    void set_genesis(const currency::block& genesis_block);
    crypto::hash get_genesis_hash();
    ...
    void dump_trunsfers(std::ostream &ss, bool verbose = true) const;
    std::string dump_trunsfers(bool verbose = false) const;
    void dump_key_images(std::ostream& ss);
    ...
    void prepare_transaction(const construct_tx_param_t& ctp, finalize_tx_param& ftp, const currency::transaction& tx_for_mode_separate = currency::transaction());
    void finalize_transaction(const finalize_tx_param& ftp, currency::transaction& tx, crypto::secret_key& tx_key, bool broadcast_tx);

    void print_free_transfers(std::ostream &s);
    void print_locked_transfers(std::ostream &s);
...
private:
    ...
    void handle_pulled_blocks(size_t& blocks_added, currency::COMMAND_RPC_GET_BLOCKS_DIRECT::response& blocks);
    ...
    bool build_escrow_proposal(bc_services::contract_private_details& ecrow_details, uint64_t fee, uint64_t unlock_time, currency::tx_service_attachment& att, std::vector<uint64_t>& selected_indices);
    bool prepare_tx_sources(size_t fake_outputs_count, std::vector<currency::tx_source_entry>& sources, std::vector<uint64_t>& selected_indices, uint64_t& found_money);
    bool prepare_tx_sources(uint64_t needed_money, size_t fake_outputs_count, uint64_t dust_threshold, std::vector<currency::tx_source_entry>& sources, std::vector<uint64_t>& selected_indices, uint64_t& found_money);
      ...
      std::vector<currency::tx_destination_entry>& final_detinations)
    {
      ...
      uint64_t dust = 0;
      destination_split_strategy(dsts, change_dts, dust_policy.dust_threshold, final_detinations, dust,
        WALLET_MAX_ALLOWED_OUTPUT_AMOUNT);
      ...
      if (final_detinations.size() > 10)
  {
    ...
    void add_sent_tx_detailed_info(const currency::transaction& tx,
      ...
      const std::vector<uint64_t>& selected_indicies);
    void mark_transfers_as_spent(const std::vector<uint64_t>& selected_transfers, const std::string& reason = std::string(), bool throw_if_flag_already_set = false);
    void mark_transfers_as_cold_sign_reserved(const std::vector<uint64_t>& selected_transfers);
    ...
    key_images_container m_pending_key_images;
    std::unordered_map<size_t, uint64_t> m_cold_sign_reserves;
    ...
    percent_calculator m_percent_calculator;
...
namespace tools
{
    ...
    const currency::core_runtime_config &runtime_config)
  {
    ...
    for (size_t i = 0; i != cxt.sp.pos_entries.size(); i++)
    {
      ...
      while(step <= POS_SCAN_WINDOW)
      {
        ...
        currency::build_kernel(cxt.sp.pos_entries[i].keyimage, sk, cxt.sm, ts);
        coindays_weight = currency::get_coinday_weight(cxt.sp.pos_entries[i].amount);
  ...
  //----------------------------------------------------------------------------------------------------
  template<typename callback_t>
  void wallet2::enumerate_transfers_history(callback_t cb, bool forward) const
  {
    common::enumerate_container(m_transfer_history, std::forward<callback_t>(cb), forward);
  }
  //----------------------------------------------------------------------------------------------------
  template<typename callback_t>
  void wallet2::enumerate_unconfirmed_transfers(callback_t cb) const
  {
    common::enumerate_container(m_unconfirmed_txs, std::forward<callback_t>(cb));
  }
  //----------------------------------------------------------------------------------------------------
}
