...
namespace tools
{
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool default_http_core_proxy::call_COMMAND_RPC_GET_STATIC_INFO(const currency::COMMAND_RPC_GET_STATIC_INFO::request& req, currency::COMMAND_RPC_GET_STATIC_INFO::response& res)
  {
    return net_utils::invoke_http_json_rpc("/json_rpc", "getstaticinfo", req, res, m_http_client);
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  {
    return epee::net_utils::invoke_http_json_remote_command2(m_daemon_address + "/force_relay", req, res, m_http_client, WALLET_RCP_CONNECTION_TIMEOUT);
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  bool default_http_core_proxy::call_COMMAND_RPC_GET_BLOCKS_DETAILS(const currency::COMMAND_RPC_GET_BLOCKS_DETAILS::request& req, currency::COMMAND_RPC_GET_BLOCKS_DETAILS::response& res)
  {
    ...
    return epee::net_utils::invoke_http_json_rpc("/json_rpc", "get_blocks_details", req, res, m_http_client);
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  {
    return epee::net_utils::invoke_http_json_rpc("/json_rpc", "get_current_core_tx_expiration_median", req, res, m_http_client);
  }
  //------------------------------------------------------------------------------------------------------------------------------
  ...
  //------------------------------------------------------------------------------------------------------------------------------
  bool default_http_core_proxy::get_transfer_address(const std::string& adr_str, currency::account_public_address& addr, currency::bc_payment_id& payment_id)
  {
