...
#define SECONDS_IN_ONE_DAY (60*60*24)
  ...
  bool is_offer_matched_by_filter(const offer_details_ex& o, const core_offers_filter& of, uint64_t current_time)
  {
    ...
    if (!of.keyword.empty())
    {
      ...
      auto check_func = [&keyword_lowcase] (const std::vector<const std::string *> &values) -> bool
      {
        for (auto value : values)
          if (to_lower_local_w(*value).find(keyword_lowcase) != std::wstring::npos)
            return true;
        return false;
      };
      if (!check_func({ &o.bonus, &o.target, &o.location_country, &o.location_city, &o.contacts, &o.comment,
                        &o.payment_types, &o.deal_option, &o.category }))
        return false;
    }
  ...
  bool filter_offers_list(std::list<bc_services::offer_details_ex>& offers, const bc_services::core_offers_filter& filter, uint64_t current_core_time)
  {
    ...
    auto it = std::next(offers.begin(), std::min<uint64_t>(filter.offset, offers.size()));
    offers.erase(offers.begin(), it);
    // cut limit
    it = std::next(offers.begin(), std::min<uint64_t>(filter.limit, offers.size()));
