...
namespace currency
{
  ...
  public:
    ...
    bool add_tx(const transaction &tx, uint64_t min_fee, const crypto::hash &id, uint64_t blob_size, tx_verification_context& tvc, bool kept_by_block, bool from_core = false);
    bool add_tx(const transaction &tx, uint64_t min_fee, tx_verification_context& tvc, bool kept_by_block, bool from_core = false);
  ...
  private:
    ...
    bool process_cancel_offer_rules(const transaction& tx, bool to_validate);
    ...
    using transaction_region_tr = tools::db::transaction_region<decltype(m_db_transactions)>;
    using transaction_region_readonly = tools::db::transaction_region_readonly<decltype(m_db)>;
