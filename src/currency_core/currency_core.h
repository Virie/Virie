...
namespace currency
{
  ...
  class stop_senders : public i_stop_senders
  {
  public:
      bool from_free_space(const std::string& message) override
      {
        m_free_space.store(true);
        return translate_signal(message, m_ignore_free_space);
      }
      bool from_time_sync(const std::string& message) override
      {
        m_time_sync.store(true);
        return translate_signal(message, m_ignore_time_sync);
      }
      void set_stop_handler(i_stop_handler * sh)
      {
        CRITICAL_REGION_LOCAL(m_stop_handler_lock);
        m_stop_handler = sh;
      }
      virtual void set_ignore_free_space(bool ignore) override
      {
        m_ignore_free_space = ignore;
      }
      virtual void set_ignore_time_sync(bool ignore) override
      {
        m_ignore_time_sync = ignore;
      }
      virtual bool was_event_free_space() override
      {
        return m_free_space.load();
      }
      virtual bool was_event_time_sync() override
      {
        return m_free_space.load();
      }

  private:
      bool translate_signal(const std::string& message, bool ignore)
      {
        if (ignore)
        {
          LOG_PRINT_RED_L0(ENDL << ENDL << "IGNORED: " << message << ENDL << ENDL);
          return false;
        }
        LOG_PRINT_RED_L0(ENDL << ENDL << message << ENDL << ENDL);
        {
          CRITICAL_REGION_LOCAL(m_stop_handler_lock);
          if (m_stop_handler == nullptr)
            return false;
           m_stop_handler->stop_handling();
        }
        return true;
      }
      critical_section m_stop_handler_lock;

      bool m_ignore_free_space{false};
      bool m_ignore_time_sync{false};
      i_stop_handler * m_stop_handler{nullptr};
      std::atomic<bool> m_free_space{false};
      std::atomic<bool> m_time_sync{false};
  };
  /************************************************************************/
  /*                                                                      */
  /************************************************************************/
   ...
   public:
     ...
     //-------------------- i_miner_handler -----------------------
     bool handle_block_found(const block& b, block_verification_context* p_verification_result = nullptr) override;
     bool get_block_template(block& b, const account_public_address& adr, const account_public_address& stakeholder_address,
       wide_difficulty_type& diffic, uint64_t& height, const blobdata& ex_nonce, bool pos = false, const pos_entry& pe = pos_entry()) override;
     bool get_block_template(const create_block_template_params& params, create_block_template_response& resp);
...
//     void get_all_known_block_ids(std::list<crypto::hash> &main, std::list<crypto::hash> &alt, std::list<crypto::hash> &invalid);
     ...
     i_stop_senders& get_ref_stop_senders() { return m_stop_senders; }
     bool set_checkpoints(checkpoints&& chk_pts);
     void validate_checkpoints();
     ...
     //debug functions
     void print_blockchain(std::ostream &s, uint64_t start_index, uint64_t end_index);
