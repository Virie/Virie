...
namespace currency
{
  ...
  public:
    ...
    bool validate_checkpoints(uint64_t &valid_blockchain_size);
    ...
    bool get_block_by_hash(const crypto::hash &bl_id, block &blk) const;
    bool get_block_extended_info_by_height(uint64_t h, block_extended_info &blk) const;
    bool get_block_extended_info_by_hash(const crypto::hash &bl_id, block_extended_info &blk) const;
    ...
    bool validate_tx_service_attachments_in_services(const tx_service_attachment& a, size_t i, const transaction& tx)const;
    ...
    inline void add_event_handler(const std::string& id, currency::i_core_event_handler* event_handler)
    {
      m_event_handlers.add(id, event_handler);
    }

    inline bool del_event_handler(const std::string& id)
    {
      return m_event_handlers.del(id);
    }
    currency::i_core_event_handler* get_event_handler()
    {
    ...
    uint64_t get_alias_cost(const std::string& alias)const;
    ...
    {
      core_event_v e(ed);
      m_event_handlers.on_core_event(event_name, e);
    }
    ...
    void print_blockchain(std::ostream &s, uint64_t start_index, uint64_t end_index) const ;
    ...
    void print_db_cache_perfeormance_data(std::ostream &s) const;
  ...
  private:
    ...
    */
    ...
    using transaction_region = tools::db::transaction_region<decltype(m_db)>;
    ...
    currency::core_event_handlers m_event_handlers{};
    ...
    //just informational 
    mutable difficulty_cache m_diff_cache;
    ...
    bool switch_to_alternative_blockchain(alt_chain_type& alt_chain, bool &had_rollback);
    void purge_alt_block_txs_hashs(const block& b);
    void add_alt_block_txs_hashs(const block& b);   
    bool pop_block_from_blockchain(bool add_txs_to_pool = true);
    bool purge_block_data_from_blockchain(const block& b, size_t processed_tx_count);
    bool purge_block_data_from_blockchain(const block& b, size_t processed_tx_count, uint64_t& fee, bool add_txs_to_pool = true);
    bool purge_transaction_from_blockchain(const crypto::hash& tx_id, uint64_t& fee, bool add_to_pool = true);
    ...
    bool build_alt_subchain(alt_chain_type &alt_chain, const block& b, const crypto::hash& id);
    bool handle_alternative_block(const block& b, const crypto::hash& id, crypto::hash &proof, alt_chain_type &alt_chain, block_verification_context& bvc);
    ...
    bool rollback_blockchain_switching_old(std::list<block>& original_chain, size_t rollback_height);
    bool rollback_blockchain_switching(size_t rollback_height);
    ...
    void on_block_added(const block_extended_info& bei, const crypto::hash& id, bool pos);
    ...
    bool complete_timestamps_vector(uint64_t start_height, std::vector<uint64_t>& timestamps, size_t elements_count = BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW)const;
    ...
    template<class visitor_t>
    void enum_blockchain(visitor_t& v, const alt_chain_type& alt_chain = alt_chain_type(), uint64_t split_height = 0) const;
    ...
    inline bool checked_get_height_by_id(const crypto::hash& bl_id, uint64_t& height) const
    {
      CRITICAL_REGION_LOCAL(m_read_lock);
      auto block_index_ptr = m_db_blocks_index.find(bl_id);
      if (m_db_blocks_index.end() == block_index_ptr)
        return false;
      const auto sz = m_db_blocks.size();
      const auto h = *block_index_ptr;
      CHECK_AND_ASSERT_MES(h < sz, false, "Internal error: bl_id=" << string_tools::pod_to_hex(bl_id) << " have index record with offset=" << h
                           << ", bigger then m_db_blocks.size()=" << sz);
      height = h;
      return true;
    }
  ...
  template<class visitor_t>
  void blockchain_storage::enum_blockchain(visitor_t& v, const alt_chain_type& alt_chain, uint64_t split_height) const
  {
