...
namespace currency
{
  bool bc_attachment_services_manager::add_service(i_bc_service* pservice)
  {
    if (pservice == nullptr)
    {
      LOG_ERROR("Service pservice is null");
      return false;
    }
    ...
    LOG_PRINT_L2("bc_attachment_services_manager: Added service " << pservice->get_id());
    m_services[pservice->get_id()] = pservice;
    pservice->set_core_runtime_config(m_core_runtime_config);
    pservice->set_event_handler(m_pcore_event_handler);
    return true;
  }
