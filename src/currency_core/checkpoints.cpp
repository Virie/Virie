...
namespace currency
{
  ...
  {
    return !height || (!m_points.empty() && (height <= (--m_points.end())->first));
  }
  //---------------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------------
  bool checkpoints::check_blocks(std::function<crypto::hash(uint64_t)> &&get_block_hash_func, uint64_t max_height, uint64_t &valid_blockchain_size) const
  {
    valid_blockchain_size = 1;
    for (auto &kv : m_points) {
      if (kv.first > max_height)
        break;
      if (get_block_hash_func(kv.first) != kv.second)
        return false;
      valid_blockchain_size = kv.first + 1;
    }
    return true;
  }
}
