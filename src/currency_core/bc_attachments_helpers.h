...
namespace bc_services
{
  ...
  bool pack_attachment_as_json(const t_attachment_type_t& at, currency::tx_service_attachment& att)
  {
    //TODO: make json without spaces and line endings here to reduce space
    return epee::serialization::store_t_to_json(at, att.body);
  }

  template<class t_attachment_type_t>
  bool pack_attachment_as_bin(const t_attachment_type_t& at, currency::tx_service_attachment& att)
  {
    //TODO: make json without spaces and line endings here to reduce space
    return ::t_serializable_object_to_blob(at, att.body);
  }
