...
#define TX_SERVICE_ATTACHMENT_DEFLATE_BODY  static_cast<uint8_t>(1 << 1)
  ...
  struct extra_alias_entry_base
  {
    ...
    std::vector<crypto::secret_key> m_view_key; // only one or zero elements expected (std::vector is using as memory efficient container for such a case)
    std::vector<crypto::signature> m_sign;      // only one or zero elements expected (std::vector is using as memory efficient container for such a case)
    //uint8_t flags;
