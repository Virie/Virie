...
ENABLE_CHANNEL_BY_DEFAULT("tx_pool", "Transactions pool default channel.")
...
namespace currency
{
  ...
  //---------------------------------------------------------------------------------
  bool tx_memory_pool::add_tx(const transaction &tx, uint64_t min_fee, const crypto::hash &id, uint64_t blob_size, tx_verification_context& tvc, bool kept_by_block, bool from_core)
  {    
    const bool is_in_checkpoint_zone = m_blockchain.is_in_checkpoint_zone();
    TIME_MEASURE_START_PD(tx_processing_time);
    TIME_MEASURE_START_PD(check_inputs_types_supported_time);
    if (!is_in_checkpoint_zone && !check_inputs_types_supported(tx))
    {
    ...
    if (!is_in_checkpoint_zone && !from_core && !kept_by_block && m_blockchain.is_tx_expired(tx))
    {
    ...
    if (!is_in_checkpoint_zone && !from_core && !validate_alias_info(tx, kept_by_block))
    {
    ...
    if(!is_in_checkpoint_zone && !from_core && !kept_by_block)
    {
    ...
    const uint64_t tx_fee = inputs_amount - outputs_amount;
    const bool pcor = process_cancel_offer_rules(tx, !kept_by_block && !from_core);
    if (!is_in_checkpoint_zone && tx_fee < min_fee)
    {
      ...
      else
      {
        ...
        LOG_ERROR("Transaction with id= " << id << " has too small fee: " << tx_fee << ", expected fee: " << min_fee);
    ...
    auto r = do_insert_transaction(tx, id, blob_size, kept_by_block, inputs_amount - outputs_amount, ch_inp_res ? max_used_block_id : null_hash, ch_inp_res ? max_used_block_height : 0);
    CHECK_AND_ASSERT_MES(r, false, "failed to do_insert_transaction");
  ...
  bool tx_memory_pool::do_insert_transaction(const transaction &tx, const crypto::hash &id, uint64_t blob_size, bool kept_by_block, uint64_t fee, const crypto::hash& max_used_block_id, uint64_t max_used_block_height)
  {
    ...
    bool r = m_db_transactions.set(id, td);
    CHECK_AND_ASSERT_MES(r, false, "failed to m_db_transactions.set");
    if (!on_tx_add(tx, kept_by_block))
      LOG_ERROR("failed to on_tx_add");
  ...
  //---------------------------------------------------------------------------------
  bool tx_memory_pool::process_cancel_offer_rules(const transaction& tx, bool to_validate)
  {
    //TODO: this code doesn't take into account offer id in source tx
    //TODO: add scan on tx size for free transaction here
    transaction_region_tr tr(m_db_transactions, transaction_region_tr::event_t::commit);
    CHECK_AND_ASSERT_MES(tr, false, "failed to begin_transaction");
    ...
    for (const auto& a: tx.attachment)
    {
      if (a.type() == typeid(tx_service_attachment))
      {
        ...
        {
          if (to_validate && !m_blockchain.validate_tx_service_attachments_in_services(srv_at, serv_att_count, tx))
          {
            LOG_ERROR("validate_tx_service_attachments_in_services failed for an offer cancellation transaction");
            return false;
          }
    ...
    for (auto co : co_list)
    {
      if (!m_db_cancel_offer_hash.set(co.tx_id, true))
        LOG_ERROR("failed to m_db_cancel_offer_hash.set");
    }
  ...
  bool tx_memory_pool::get_aliases_from_tx_pool(std::map<std::string, size_t>& aliases)const
  {
    ...
    auto r = get_aliases_from_tx_pool(aliases_local);
    CHECK_AND_ASSERT_MES(r, false, "failed to get_aliases_from_tx_pool");
  ...
  {
    transaction_region_readonly tr(m_db);
  ...
  //---------------------------------------------------------------------------------
  bool tx_memory_pool::add_tx(const transaction &tx, uint64_t min_fee, tx_verification_context& tvc, bool kept_by_block, bool from_core)
  {
    crypto::hash tx_id = null_hash;
    uint64_t blob_size = 0;

    auto r = get_transaction_hash(tx, tx_id, blob_size);
    CHECK_AND_ASSERT_MES(r, false, "failed to get_transaction_hash");
    return add_tx(tx, min_fee, tx_id, blob_size, tvc, kept_by_block, from_core);
  }
  //---------------------------------------------------------------------------------
  bool tx_memory_pool::take_tx(const crypto::hash &id, transaction &tx, size_t& blob_size, uint64_t& fee)
  {
    ...
    fee = txe_tr->fee;
    if (!unprocess_cancel_offer_rules(txe_tr->tx))
      LOG_ERROR("failed to unprocess_cancel_offer_rules");
    m_db_transactions.erase(id);
    if (!on_tx_remove(tx, txe_tr->kept_by_block))
      LOG_ERROR("failed to on_tx_remove");
    set_taken(id);
    return true;
  }
  //---------------------------------------------------------------------------------
  ...
  {
    CRITICAL_REGION_TRY_BEGIN(m_remove_stuck_txs_lock)
    ...
    for (auto& e : to_delete)
    {
      ...
      if (!unprocess_cancel_offer_rules(e.tx))
        LOG_ERROR("failed to unprocess_cancel_offer_rules");
      if (!on_tx_remove(e.tx, e.kept_by_block))
        LOG_ERROR("failed to on_tx_remove");
    }
  ...
  bool tx_memory_pool::get_all_transactions_details(std::list<tx_rpc_extended_info>& txs) const
  {
    m_db_transactions.enumerate_items([&](uint64_t i, const crypto::hash& h, const tx_details &tx_entry)
    {
      ...
      if (!fill_tx_rpc_details(trei, tx_entry.tx, nullptr, h, tx_entry.receive_time, true))
  ...
  bool tx_memory_pool::get_transactions_details(const std::list<std::string>& ids, std::list<tx_rpc_extended_info>& txs) const
  {
    for (auto& id_str:  ids)
    {
      ...
      if (!fill_tx_rpc_details(trei, ptei->tx, nullptr, id, ptei->receive_time, false))
        LOG_ERROR("failed to fill_tx_rpc_details");
    }
    return true;
  }
  ...
  bool tx_memory_pool::add_transaction_to_black_list(const transaction& tx)
  {
    ...
    transaction_region tr(m_db);
    CHECK_AND_ASSERT_MES(tr, false, "failed to begin_transaction");
    bool r = m_db_black_tx_list.set(get_transaction_hash(tx), true);
    if (r)
      tr.will_commit();
    CHECK_AND_ASSERT_MES(r, false, "failed to m_db_black_tx_list.set");
    return true;
  }
  //---------------------------------------------------------------------------------
    ...
    for(const auto& in : tx.vin)
    {
      if (in.type() == typeid(txin_to_key))
      {
        ...
        if (ki_entry_ptr.get())
          count = *ki_entry_ptr;
        ++count;
        if (!m_db_key_images_set.set(tokey_in.k_image, count))
          LOG_ERROR("failed to m_db_key_images_set.set");
      }
    }
    return false;
  }
  ...
  bool tx_memory_pool::on_tx_add(const transaction& tx, bool kept_by_block)
  {
    ...
    auto r = insert_alias_info(tx);
    CHECK_AND_ASSERT_MES(r, false, "failed to insert_alias_info");
    return true;
  }
  //--------------------------------------------------------------------------------- 
  bool tx_memory_pool::on_tx_remove(const transaction& tx, bool kept_by_block)
  {
    ...
    auto r = remove_alias_info(tx);
    CHECK_AND_ASSERT_MES(r, false, "failed to remove_alias_info");
    return true;
  }
  //--------------------------------------------------------------------------------- 
  bool tx_memory_pool::insert_alias_info(const transaction& tx)
  {
    ...
    if (ei.m_alias.m_alias.size())
    {
      if (!m_db_alias_names.set(ei.m_alias.m_alias, true))
      {
        LOG_ERROR("failed to m_db_alias_names.set");
        r = false;
      }
      if (!m_db_alias_addresses.set(ei.m_alias.m_address, true))
      {
        LOG_ERROR("failed to m_db_alias_addresses.set");
        r = false;
      }
    }
    return r;
    ...
    uint64_t height)
  {
    ...
    std::vector<std::unique_ptr<txv>> utxs;
    utxs.reserve(m_db_transactions.size());

    //std::transform(m_transactions.begin(), m_transactions.end(), txs.begin(), [](txv &a) -> txv * { return &a; });
    //keep getting it as a values cz db items cache will keep it as unserialied object stored by smart ptrs
    m_db_transactions.enumerate_items([&](uint64_t /*i*/, const crypto::hash& k, const tx_details& tx)
    {
      utxs.push_back(std::unique_ptr<txv>(new txv(k, tx)));
      return true;
    });

    std::sort(utxs.begin(), utxs.end(), [](const std::unique_ptr<txv> &pa, const std::unique_ptr<txv> &pb) -> bool {
      const boost::multiprecision::uint128_t a = boost::multiprecision::uint128_t(pa->second.fee) * pb->second.blob_size;
      const boost::multiprecision::uint128_t b = boost::multiprecision::uint128_t(pb->second.fee) * pa->second.blob_size;
      return a > b;
    });
    ...
    for (const auto & ptx : utxs)
    {
      tx_extra_info ei = AUTO_VAL_INIT(ei);
      bool r = parse_and_validate_tx_extra(ptx->second.tx, ei);
    ...
    for (size_t i = 0; i < utxs.size(); ++i)
    {
      auto &utx = utxs[i];

      // expiration time check -- skip expired transactions
      if (is_tx_expired(utx->second.tx, tx_expiration_ts_median))
      {
          utx.reset(nullptr);
          continue;
      } 
      ...
      // alias checks
      tx_extra_info ei = AUTO_VAL_INIT(ei);
      bool r = parse_and_validate_tx_extra(utx->second.tx, ei);
      CHECK_AND_ASSERT_MES(r, false, "failed to validate transaction extra on unprocess_blockchain_tx_extra");
      if (!ei.m_alias.m_alias.empty())
      {
        const bool update_an_alias = !ei.m_alias.m_sign.empty();
        ...
        {
          utx.reset(nullptr);                                         // ...skip this tx
          continue;
        }
      }
      ...
      tx_details local_copy_txd = utx->second;
      const bool is_tx_ready_to_go_result = is_transaction_ready_to_go(local_copy_txd, utx->first);
      if (!is_tx_ready_to_go_result && 
        (local_copy_txd.last_failed_height != utx->second.last_failed_height || local_copy_txd.last_failed_id != utx->second.last_failed_id))
      {
        transaction_region_tr tr(m_db_transactions);
        CHECK_AND_ASSERT_MES(tr, false, "failed to begin_transaction");
        r = m_db_transactions.set(get_transaction_hash(local_copy_txd.tx), local_copy_txd);
        if (!r)
      ...
      if (!is_tx_ready_to_go_result || have_key_images(k_images, utx->second.tx))
      {
        utx.reset(nullptr);
        continue;
      }
      append_key_images(k_images, utx->second.tx);

      current_size += utx->second.blob_size;
      current_fee += utx->second.fee;
    ...
    for (size_t i = 0; i != utxs.size(); ++i)
    {
      ...
      if (utx)
      {
        if (i < best_position)
        {
          bl.tx_hashes.push_back(utx->first);
        }
        else if (have_attachment_service_in_container(utx->second.tx.attachment, BC_OFFERS_SERVICE_ID, BC_OFFERS_SERVICE_INSTRUCTION_DEL))
        {
          // BC_OFFERS_SERVICE_INSTRUCTION_DEL transactions has zero fee, so include them here regardless of reward effectiveness
          bl.tx_hashes.push_back(utx->first);
          total_size += utx->second.blob_size;
        }
      }
    }
    return true;
  ...
  bool tx_memory_pool::init(const std::string& config_folder)
  {
    ...
    if (!tools::create_directories_if_necessary(folder_name))
      LOG_ERROR("failed to create_directories_if_necessary");
