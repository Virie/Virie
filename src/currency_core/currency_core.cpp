...
namespace currency
{
  ...
  {
    m_stop_senders.set_stop_handler(handler);
  }
  //-----------------------------------------------------------------------------------
  bool core::set_checkpoints(checkpoints&& chk_pts)
  {
    return m_blockchain_storage.set_checkpoints(std::forward<checkpoints>(chk_pts));
  }
  //-----------------------------------------------------------------------------------
  void core::validate_checkpoints()
  {
    uint64_t valid_blockchain_size;
    if (!m_blockchain_storage.validate_checkpoints(valid_blockchain_size))
    {
      LOG_ERROR("failed to validate checkpoints");
      m_blockchain_storage.truncate_blockchain(valid_blockchain_size);
    }
  }
  //-----------------------------------------------------------------------------------
...
//     }
    ...
    r = load_state_data();
    CHECK_AND_ASSERT_MES(r, false, "Failed to load state data");
  ...
  bool core::handle_incoming_tx(const blobdata& tx_blob, tx_verification_context& tvc, bool kept_by_block)
  {
    ...
    if(tx_blob.size() >= m_blockchain_storage.get_current_comulative_blocksize_limit() - CURRENCY_COINBASE_BLOB_RESERVED_SIZE)
    {
      LOG_PRINT_RED_L0("tx has too big size " << tx_blob.size() << ", expected no bigger than " << m_blockchain_storage.get_current_comulative_blocksize_limit() - CURRENCY_COINBASE_BLOB_RESERVED_SIZE);
      tvc.m_verification_failed = true;
      return false;
    }
  ...
  bool core::add_new_tx(const transaction& tx, const crypto::hash& tx_hash, size_t blob_size, tx_verification_context& tvc, bool kept_by_block)
  {
    ...
    const uint64_t tx_min_fee = currency::get_tx_minimum_fee(get_current_blockchain_size());
    return m_mempool.add_tx(tx, tx_min_fee, tx_hash, blob_size, tvc, kept_by_block);
  }
  //-----------------------------------------------------------------------------------------------
  ...
  //-----------------------------------------------------------------------------------------------
  void core::print_blockchain(std::ostream &s, uint64_t start_index, uint64_t end_index)
  {
    m_blockchain_storage.print_blockchain(s, start_index, end_index);
  }
  //-----------------------------------------------------------------------------------------------
...
//   }
  ...
  bool core::on_idle()
  {
    ...
    m_check_free_space_interval.do_call([this]()
    {
        uint64_t available;
        TRY_ENTRY()
        {
          available = tools::get_free_space(m_config_folder);
        }
        CATCH_ENTRY("core::on_idle tools::get_free_space exception ", true);
        if (available >= MINIMUM_REQUIRED_FREE_SPACE_BYTES)
          return true;
        m_stop_senders.from_free_space("Free space at data directory is critically low ("
          + std::to_string(available / (1024 * 1024))
          + " MB, while "
          + std::to_string(MINIMUM_REQUIRED_FREE_SPACE_BYTES / (1024 * 1024))
          + " MB is required), daemon will stop immediately");
        return true;
    });
    m_miner.on_idle();
    m_mempool.on_idle();
