...
ENABLE_CHANNEL_BY_DEFAULT("stratum", "Stratum server default channel.")
...
#define VDIFF_VARIANCE_PERCENT_DEFAULT 25 // %
  ...
  const command_line::arg_descriptor<std::string> arg_stratum_miner_address = {"stratum-miner-address",     "Stratum server: miner address. All workers"
    ...
    " If set here - they're allowed to log in with username '" WORKER_ALLOWED_USERNAME "' instead of address.", "", command_line::flag_t::not_use_default };
...
//==============================================================================================================================
  ...
  struct stratum_protocol_handler_config : public i_blockchain_update_listener
  {
    ...
    void add_protocol_handler(protocol_handler_t* p_ph)
    {
      ...
      CRITICAL_REGION_END();
      LOG_PRINT_CC(p_ph->get_context(), "stratum_protocol_handler_config: protocol handler added", LOG_LEVEL_MAX);
    ...
    void remove_protocol_handler(protocol_handler_t* p_ph)
    {
      ...
      CRITICAL_REGION_END();
      LOG_PRINT_CC(p_ph->get_context(), "stratum_protocol_handler_config: protocol handler removed", LOG_LEVEL_MAX);
    ...
    bool update_block_template(bool enforce_update = false)
    {
      ...
      LOG_PRINT("stratum_protocol_handler_config::update_block_template(" << (enforce_update ? "true" : "false") << ")", LOG_LEVEL_MAX);
    ...
    {
      LOG_PRINT("stratum_protocol_handler_config::set_work_for_all_workers()", LOG_LEVEL_MAX);
    ...
    std::string get_work_json(const wide_difficulty_type& worker_difficulty)
    {
      ...
      ethash_h256 seed_hash = ethash_get_seedhash(m_block_template_height);
    ...
    {
      LOG_PRINT_CC(p_ph->get_context(), "stratum_protocol_handler_config::update_work()", LOG_LEVEL_MAX);
      bool updated = false;
      if (is_core_syncronized())
      {
    ...
    bool handle_login(protocol_handler_t* p_ph, const jsonrpc_id_t& id, const std::string& user_str, const std::string& pass_str, const std::string& worker_str, uint64_t start_difficulty)
    {
      ...
      if (user_str == WORKER_ALLOWED_USERNAME)
      {
        ...
        {
          error = true;
          error_stream << "trying to log in with '" WORKER_ALLOWED_USERNAME "' username, while mining address WAS NOT previously set in daemon. Set mining address in daemon OR use it as a username.";
        }
      }
      else
      {
        ...
        {
          error = true;
          error_stream << "can't parse wallet address from username: " << user_str << ".";
        }

        if (m_miner_addr != null_pub_addr && address != m_miner_addr)
        {
          error = true;
          error_stream << "wallet address " << user_str << " doesn't match the address previously set in daemon and/or other workers.";
        }
      ...
      if (error)
      {
        const std::string& error_str = error_stream.str();
        LP_CC_WORKER_RED(p_ph->get_context(), error_str << " Connection will be dropped.", LOG_LEVEL_0);
        p_ph->send_response_error(id, JSONRPC_ERROR_CODE_DEFAULT, error_str);
        return false;
      }
  ...
  public:
    ...
    {
      LOG_PRINT_CC(m_context, "stratum_protocol_handler::ctor()", LOG_LEVEL_MAX);
    }
    ...
    ~stratum_protocol_handler()
    {
      ...
      LOG_PRINT_CC(m_context, "stratum_protocol_handler::dtor()", LOG_LEVEL_MAX);
    }
    ...
    bool handle_recv(const void* p_data, size_t data_size)
    {
      ...
      LP_CC_WORKER(m_context, "data received: " << data_size << " bytes:" << ENDL << std::string(str, data_size), LOG_LEVEL_MAX);
    ...
    bool handle_json_request(epee::serialization::portable_storage& ps, const std::string& json)
    {
      ...
      if (ps_get_value_noexcept(ps, "method", method, nullptr))
      {
        ...
        if (handler_it == m_methods_handlers.end())
        {
          ...
          LP_CC_WORKER_RED(m_context, error_str, LOG_LEVEL_1);
          send_response_error(id, JSONRPC_ERROR_CODE_METHOD_NOT_FOUND, error_str);
          return false;
        }
    ...
    void send(const std::string& data)
    {
      ...
      LOG_PRINT_CC(m_context, "DATA sent >>>>>>>>>>>>> " << ENDL << data, LOG_LEVEL_MAX);
    }
    ...
    {
      LOG_PRINT_CC(m_context, "stratum_protocol_handler::after_init_connection()", LOG_LEVEL_MAX);
      if (!m_connection_initialized)
      {
    ...
    {
      LOG_PRINT_CC(m_context, "stratum_protocol_handler::release_protocol()", LOG_LEVEL_MAX);
...
//------------------------------------------------------------------------------------------------------------------------------
std::string stratum_server::get_binded_address() const
{
  return m_impl->server.get_binded_address();
}
//------------------------------------------------------------------------------------------------------------------------------
uint32_t stratum_server::get_binded_port() const
{
  return m_impl->server.get_binded_port();
}
//------------------------------------------------------------------------------------------------------------------------------
