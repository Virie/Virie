...
#include "ethereum/libethash/sha3.h"
#include "ethereum/libethash/ethash.h"
...
//------------------------------------------------------------------------------------------------------------------------------
  ...
  /*bool uint64_from_net_format(const std::string& str, uint64_t& result)
  {
    if (!epee::string_tools::hex_to_pod(trim_0x(str), result))
      return false;

    result = swap64be(result);
    return true;
  }*/

  uint64_t epoch_by_seedhash(const ethash_h256_t& seed_hash)
  {
	  ethash_h256_t ret;
	  ethash_h256_reset(&ret);
	  for (uint32_t i = 0; i < 2016; ++i) // 2016 epoches will be enough until 2038
    {
      if (memcmp(&ret, &seed_hash, sizeof ret) == 0)
        return i;
...
inline std::ostream &operator <<(std::ostream &o, const ethash_h256_t &v) { return print256(o, v); }
