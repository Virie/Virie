...
struct check_transaction_region
{
  typedef tools::db::cached_key_value_accessor<uint64_t, std::string, true, true> accessor_type;

  using transaction_region = tools::db::transaction_region<accessor_type>;
  ...
  static bool run_test(accessor_type& acc)
  {
    ...
    {
      transaction_region tr(acc);
      acc.set(key1, data1);
    }
    CHECK_AND_ASSERT_MES(acc.find(key1) == acc.end(), false, "");

    {
      transaction_region tr(acc, transaction_region::event_t::abort, [&flag] () { flag = true; });
      tr.will_commit();
      acc.set(key1, data1);
    }
    CHECK_AND_ASSERT_MES(*acc.get(key1) == data1, false, "");
    CHECK_AND_ASSERT_MES(flag, false, "");
    ...
    CHECK_AND_ASSERT_MES(*acc.get(key2) == data2, false, "");

    {
      transaction_region tr(acc, transaction_region::event_t::abort, [&flag] () { flag = false; });
      acc.erase(key1);
      tr.commit();
    }
    CHECK_AND_ASSERT_MES(acc.find(key1) == acc.end(), false, "");
    ...
    CHECK_AND_ASSERT_MES(acc.find(key2) != acc.end(), false, "");
...
int main(int argc, char* argv[])
{
  ...
  if (!prepare_db_and_run_test<check_transaction_region>())
    return 0;
