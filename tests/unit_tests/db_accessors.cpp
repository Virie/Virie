...
struct naive_median
{
  enum { WINDOW_SIZE = ALIAS_COST_PERIOD };
...
struct bcs_stub_t
{
  ...
  {
    uint64_t period = is_pos_allowed() ? ALIAS_COST_PERIOD : ALIAS_COST_PERIOD / 2;
...
#define PERIOD_DISABLED   0xffffffffffffffffLL
    ...
    uint64_t purge_recent_period = is_pos_allowed() ? ALIAS_COST_RECENT_PERIOD : ALIAS_COST_RECENT_PERIOD / 2;
...
TEST(db_accessor_tests, median_db_cache_test)
{
  ...
  if (!bcs_loaded || bcs_stub.m_tx_fee_median.get_median() == 0)
  {
    ...
    size_t initial_blocks_count = ALIAS_COST_RECENT_PERIOD + 20;
  ...
  for (size_t step = 0; step < steps; ++step)
  {
    ...
    {
      uint64_t normal_window = ALIAS_COST_PERIOD;
      uint64_t recent_window = ALIAS_COST_RECENT_PERIOD;
