...
namespace
{
  ...
  std::string test_keys_addr_str = "HinoMuPdnJCdNeM6dfiBFL7kqund3HYGvMBF3ttsNd9SfzgYB6L7ep1Yg1osYJzLdaKAYSLVh6e6jKnAuzj3bw1oGwmmPYn"; // correct str address depends on CURRENCY_PUBLIC_ADDRESS_BASE58_PREFIX value
}
...
TEST(integ_address, payment_id_sizes)
{
  ...
  {
    currency::bc_payment_id payment_id{std::string(pid_size, ' ')};
    if (pid_size > 0)
      crypto::generate_random_bytes(pid_size, &payment_id.id.front());
    ...
    currency::bc_payment_id integrated_payment_id;
