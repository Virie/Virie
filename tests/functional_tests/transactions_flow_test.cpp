...
bool do_register_alias(tools::wallet2& w1, const std::string& alias_name, const std::string& comment, const account_public_address& address, transaction& tx)
{
  try
  {
    ...
    w1.request_alias_registration(eae, tx, TX_DEFAULT_FEE,
                                  currency::get_alias_cost_from_fee(eae.m_alias,
                                                                    TX_DEFAULT_FEE * 10));
    uint64_t ticks_for_tx = epee::misc_utils::get_tick_count() - ticks;
...
{
  flow_test_context(tools::wallet2& w_) :w(w_), pause(false)
  {}
  void stop() { w.stop(); }
  tools::wallet2& w;
...
void wait_unlock_money(tools::wallet2& w, flow_test_context& control)
{
  ...
  while (!control.pause && !control.w.is_stopped())
  {
    LOG_PRINT_GREEN("Wait iteration " << wait_count++ << " ...", LOG_LEVEL_0);
    misc_utils::sleep_no_w(1000);
    w.refresh(blocks_fetched);
    if (blocks_fetched)
    {
...
void wait_receive_and_unlock_half_balance(tools::wallet2& w, flow_test_context& control)
{
  ...
  while (w.unlocked_balance() < w.balance()/2 && !control.pause && !control.w.is_stopped())
  {
    LOG_PRINT_GREEN("Wait iteration " << wait_count++ << " ...", LOG_LEVEL_0);
    misc_utils::sleep_no_w(1000);
    w.refresh(blocks_fetched);
  ...
  size_t mix_in_factor, size_t action, uint64_t max_tx_in_pool)
{
  ...
  size_t blocks_fetched = 0;
  if(!w1.refresh_noexept(blocks_fetched))
  {
  ...
  while (!control.w.is_stopped())
  {
    ...
    {
      sent_amount = currency::get_alias_cost_from_fee(get_random_hex_text(12), TX_DEFAULT_FEE);
