                                   ...
                                   const core_runtime_config& cc)
{
  ...
  for (auto a : accs)
  {
    ...
    currency::block genesis_block;
    generate_genesis_block(genesis_block);
    wallets.back()->set_genesis(genesis_block);
...
uint64_t test_generator::get_timestamps_median(const blockchain_vector& blck_chain, size_t window_size /* = BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW */)
{
  ...
  size_t start_index = blck_chain.size() - std::min(blck_chain.size(), window_size);
  size_t offset = start_index == 0 ? 1 : start_index;
  timestamps.reserve(blck_chain.size() - offset);
  for(; offset < blck_chain.size(); offset++)
  {
    timestamps.push_back(blck_chain[offset]->b.timestamp);
  }
...
bool test_generator::init_test_wallet(const currency::account_base& account, const currency::block& genesis_block, std::shared_ptr<tools::wallet2> &result)
{
...
bool test_generator::refresh_test_wallet(const std::vector<test_event_entry>& events, tools::wallet2* w, const crypto::hash& top_block_hash, size_t expected_blocks_to_be_fetched)
{
  ...
  bool r = w->refresh_noexept(blocks_fetched);
...
bool test_generator::construct_pow_block_with_alias_info_in_coinbase(const account_base& acc, const block& prev_block, const extra_alias_entry& ai, block& b)
{
  return construct_block_gentime_with_coinbase_cb(prev_block, acc, [&acc, &ai](transaction& miner_tx){
    ...
    if (ai.m_sign.empty())
    {
      ...
      uint64_t alias_cost = get_alias_cost_from_fee(ai.m_alias, TX_DEFAULT_FEE);
...
void print_wallet(const tools::wallet2& w, const char* account_name)
{
  uint64_t total, unlocked, awaiting_in, awaiting_out, mined;
...
bool calculate_amounts_many_outs_have_and_no_outs_have(const uint64_t first_blocks_reward, uint64_t& amount_many_outs_have, uint64_t& amount_no_outs_have)
{
  ...
  decompose_amount_into_digits(first_blocks_reward, DEFAULT_DUST_THRESHOLD, [&digits](uint64_t chunk){ digits.insert(chunk); }, [](uint64_t dust) { /* nope */ });
