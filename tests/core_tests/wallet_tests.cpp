...
bool gen_wallet_refreshing_on_chain_switch_2::generate(std::vector<test_event_entry>& events) const
{
  ...
  // send some money to alice                                                                                       
  MAKE_TX_FEE(events, tx_0, miner_acc, alice_acc, MK_TEST_COINS(200), TX_DEFAULT_FEE, blk_0r);                // 2N+1
  MAKE_NEXT_BLOCK_TX1(events, blk_1, blk_0r, miner_acc, tx_0);                                                      // 2N+1

  MAKE_TX_FEE(events, tx_1, miner_acc, alice_acc, MK_TEST_COINS(50), TX_DEFAULT_FEE, blk_0r);                 // 2N+3
  ...
  CHECK_TEST_WALLET_BALANCE_AT_GEN_TIME_FULL(alice_wlt, MK_TEST_COINS(250), 0, 0, 0, 0);
  ...
  CHECK_TEST_WALLET_BALANCE_AT_GEN_TIME_FULL(alice_wlt, MK_TEST_COINS(250), 0, 0, MK_TEST_COINS(50), 0);
  DO_CALLBACK_PARAMS(events, "check_balance", params_check_balance(alice_acc_idx, MK_TEST_COINS(250), 0, 0, MK_TEST_COINS(50), 0));            // 2N+7
  ...
  CHECK_TEST_WALLET_BALANCE_AT_GEN_TIME_FULL(alice_wlt, MK_TEST_COINS(250), 0, 0, MK_TEST_COINS(250), 0);
  DO_CALLBACK_PARAMS(events, "check_balance", params_check_balance(alice_acc_idx, MK_TEST_COINS(250), 0, 0, MK_TEST_COINS(250), 0));
...
bool gen_wallet_unconfirmed_outdated_tx::generate(std::vector<test_event_entry>& events) const
{
  ...
  REWIND_BLOCKS_N_WITH_TIME(events, blk_0r, blk_0, miner_acc, CURRENCY_MINED_MONEY_UNLOCK_WINDOW + ADD_BLOCKS_FOR_TX);       // 2N (N = CURRENCY_MINED_MONEY_UNLOCK_WINDOW + ADD_BLOCK_FOR_TX)

  // create tx_0 like it's 20 sec after blk_0r
  events.push_back(event_core_time(blk_0r.timestamp + 20));                                                                  // 2N+1
  MAKE_TX_FEE(events, tx_0, miner_acc, alice_acc, MK_TEST_COINS(200), TX_DEFAULT_FEE, blk_0r);                         // 2N+2

  // check balances and populate unconfirmed tx list in a wallet
  DO_CALLBACK(events, "c1");                                                                                                 // 2N+3

  // create tx_1 like it's 25 sec after blk_0r
  events.push_back(event_core_time(blk_0r.timestamp + 25));                                                                  // 2N+4
  MAKE_TX_FEE(events, tx_1, miner_acc, alice_acc, MK_TEST_COINS(500), TX_DEFAULT_FEE, blk_0r);                         // 2N+5

  // check balances and populate unconfirmed tx list in a wallet
  DO_CALLBACK(events, "c2");                                                                                                 // 2N+6

  // add a block with no txs, so the next refresh in the wallet triggers unconfirmed txs cleaning
  MAKE_NEXT_BLOCK(events, blk_1, blk_0r, miner_acc);                                                                         // 2N+7

  // forward the time a lot, tx_0 should now be beyound the allowed livetime limit
  events.push_back(event_core_time(blk_0r.timestamp + 23 + CURRENCY_MEMPOOL_TX_LIVETIME));                                   // 2N+8

  // check again
  DO_CALLBACK(events, "c3");                                                                                                 // 2N+9
...
bool gen_wallet_unconfirmed_outdated_tx::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  bool has_alias;
  alice_wlt->refresh(blocks_fetched);
  CHECK_AND_ASSERT_MES(blocks_fetched == (CURRENCY_MINED_MONEY_UNLOCK_WINDOW + ADD_BLOCKS_FOR_TX), false, "Incorrect numbers of blocks fetched");
  alice_wlt->scan_tx_pool(has_alias);
...
bool gen_wallet_payment_id::generate(std::vector<test_event_entry>& events) const
{
  ...
  m_payment_id.id = epee::string_tools::pod_to_hex(get_block_hash(blk_0)); // use genesis hash as payment id for instance
  ...
  const currency::bc_payment_id empty_payments_id{};
  std::vector<currency::attachment_v> attachment_empty_payment_id;
  r = currency::set_payment_id_to_tx(attachment_empty_payment_id, empty_payments_id);
  ...
  payments.clear();
  alice_wlt->get_payments(empty_payments_id, payments);
...
bool gen_wallet_oversized_payment_id::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  //
  currency::bc_payment_id payment_id{std::string(BC_PAYMENT_ID_SERVICE_SIZE_MAX, 'x')};
  crypto::generate_random_bytes(payment_id.id.size(), &payment_id.id.front());
  ...
  //
  payment_id.id.resize(BC_PAYMENT_ID_SERVICE_SIZE_MAX + 1, 'x');
  crypto::generate_random_bytes(payment_id.id.size(), &payment_id.id.front());
...
bool gen_wallet_transfers_and_outdated_unconfirmed_txs::generate(std::vector<test_event_entry>& events) const
{
  ...
  REWIND_BLOCKS_N(events, blk_0r, blk_0, miner_acc, CURRENCY_MINED_MONEY_UNLOCK_WINDOW + ADD_BLOCKS_FOR_TX);
  ...
  CREATE_TEST_WALLET(alice_wlt, alice_acc, blk_0);
  REFRESH_TEST_WALLET_AT_GEN_TIME(events, alice_wlt, blk_1r, CURRENCY_MINED_MONEY_UNLOCK_WINDOW + 1 + WALLET_DEFAULT_TX_SPENDABLE_AGE + ADD_BLOCKS_FOR_TX);
...
bool gen_wallet_alias_and_unconfirmed_txs::generate(std::vector<test_event_entry>& events) const
{
  ...
  MAKE_TX_FEE_MIX_ATTR_EXTRA(events, tx_alice_alias, miner_acc, null_account,
                             get_alias_cost_from_fee(ai.m_alias, TX_DEFAULT_FEE), TX_DEFAULT_FEE, 0, blk_0r, 0, std::vector<currency::extra_v>({ai }), true);

  uint64_t amount = get_alias_cost_from_fee(
    std::string(ALIAS_MINIMUM_PUBLIC_SHORT_NAME_ALLOWED, 'a'), TX_DEFAULT_FEE);
...
bool gen_wallet_alias_and_unconfirmed_txs::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  uint64_t alias_reward = get_alias_cost_from_fee(ai.m_alias, TX_DEFAULT_FEE);
...
bool gen_wallet_alias_via_special_wallet_funcs::generate(std::vector<test_event_entry>& events) const
{
  ...
  uint64_t biggest_alias_reward = get_alias_cost_from_fee("a", TX_DEFAULT_FEE);
...
bool gen_wallet_offers_size_limit::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  log_space::get_set_log_detalisation_level(true, LOG_LEVEL_MIN);
...
bool gen_wallet_selecting_pos_entries::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  size_t blocks_fetched = 0;
  alice_wlt->refresh(blocks_fetched);
  CHECK_AND_ASSERT_MES(blocks_fetched == CURRENCY_MINED_MONEY_UNLOCK_WINDOW + 1 + WALLET_DEFAULT_TX_SPENDABLE_AGE - 2 + ADD_BLOCKS_FOR_TX, false, "Incorrect numbers of blocks fetched");
  CHECK_AND_ASSERT_MES(c.get_pool_transactions_count() == 0, false, "Incorrect txs count in the pool");
  ...
  CHECK_AND_ASSERT_MES(c.get_current_blockchain_size() == CURRENCY_MINED_MONEY_UNLOCK_WINDOW + 1 + WALLET_DEFAULT_TX_SPENDABLE_AGE - 1 + ADD_BLOCKS_FOR_TX, false, "Incorrect current blockchain height");
...
bool premine_wallet_test::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  {
    LOG_PRINT_CYAN(ENDL << " =+=+=+=+=+=+=+ wallet file: " << epee::string_encoding::wstring_to_utf8(wallet_files[i].filename) << " =+=+=+=+=+=+=+ " << ENDL, LOG_LEVEL_0);
    try{
    ...
    {
      LOG_ERROR("Got exception: " << e.what() << ENDL << " Wallet file: " << epee::string_encoding::wstring_to_utf8(wallet_files[i].filename));
      result = false;
    }
  }
...
bool mined_balance_wallet_test::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_AND_ASSERT_MES(is_pos_coinbase(b.miner_tx), false, "Invalid PoS coinbase tx");
  ...
  n = bcs.get_current_blockchain_size();
  r = miner_wlt->try_mint_pos(alice_wlt->get_account().get_public_address());
  CHECK_AND_ASSERT_MES(r && bcs.get_current_blockchain_size() > n, false, "can't mint a PoS block");
  ...
  coinbase_outs_amount = get_outs_money_amount(b.miner_tx);
  stake_amount = boost::get<txin_to_key>(b.miner_tx.vin[1]).amount;
  CHECK_AND_ASSERT_MES(coinbase_outs_amount > stake_amount, false, "coinbase_outs_amount = " << coinbase_outs_amount << ", stake_amount = " << stake_amount << " : invalid condition");

  auto reward = coinbase_outs_amount - stake_amount;

  miner_wlt->refresh();
  alice_wlt->refresh();

  CHECK_AND_ASSERT_MES(check_balance_via_wallet(*miner_wlt.get(), "miner", miner_mined_money, miner_mined_money), false, "wrong balance");
  CHECK_AND_ASSERT_MES(check_balance_via_wallet(*alice_wlt.get(), "alice", reward, 0), false, "wrong balance");
...
wallet_watch_only::wallet_watch_only()
{
  REGISTER_CALLBACK_METHOD(wallet_watch_only, c1)
}

bool wallet_watch_only::generate(std::vector<test_event_entry>& events) const
{
  m_accounts.resize(TOTAL_ACCS_COUNT);
  m_accounts[MINER_ACC_IDX].generate();
  m_accounts[ALICE_ACC_IDX].generate();
  m_alice_acc_wo = m_accounts[ALICE_ACC_IDX];
  m_alice_acc_wo.make_account_watch_only();
  CHECK_AND_ASSERT_MES(m_alice_acc_wo.get_keys().m_spend_secret_key == currency::null_skey, false, "secret spend key of watch only account is not null");
  CHECK_AND_ASSERT_MES(m_alice_acc_wo.get_restore_data().empty(), false, "restore data of watch only account is not empty");
  m_accounts[BOB_ACC_IDX].generate();
...
bool wallet_watch_only::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  auto miner_wlt = init_playtime_test_wallet(events, c, m_accounts[MINER_ACC_IDX]);
  auto alice_wlt = init_playtime_test_wallet(events, c, m_accounts[ALICE_ACC_IDX]);
  auto alice_wlt_wo = init_playtime_test_wallet(events, c, m_alice_acc_wo);
  auto bob_wlt = init_playtime_test_wallet(events, c, m_accounts[BOB_ACC_IDX]);

  CHECK_AND_ASSERT_MES(alice_wlt_wo->is_watch_only(), false, "wallet is not watch only");

  auto mine = [&miner_wlt, &c] () {
    bool r = mine_next_pow_blocks_in_playtime(miner_wlt->get_account().get_public_address(), c, CURRENCY_MINED_MONEY_UNLOCK_WINDOW);
    CHECK_AND_ASSERT_THROW_MES(r, "invalid mine_next_pow_block_in_playtime failed");
  };

  auto check_balance = [] (const char *name, const std::shared_ptr<tools::wallet2> w, const balance &b) {
    bool r = refresh_wallet_and_check_balance("", name, w, b.total, true, UINT64_MAX, b.unlocked, b.mined, b.in, b.out);
    CHECK_AND_ASSERT_THROW_MES(r, "invalid balance: " << name);
  };

  balance alice_balance { 2 * COIN, 0, 0, 2 * COIN, 0 };
  balance alice_wo_balance { 2 * COIN, 0, 0, 2 * COIN, 0 };
  balance bob_balance {};

  uint64_t send_amount = 1 * COIN - TX_DEFAULT_FEE;

  mine();
  miner_wlt->refresh();
  miner_wlt->transfer(2 * COIN, alice_wlt->get_account().get_public_address());

  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
  mine();
  {
    /// alice balance after unlocking incoming transaction from bob - equal to alice_wo balance
    alice_balance.in = 0;
    alice_balance.unlocked = alice_balance.total;
    alice_wo_balance = alice_balance;
  }
  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
  ...
  {
    transaction stub_tx;
    std::vector<tx_destination_entry> destinations { { send_amount, bob_wlt->get_account().get_public_address() } };
    alice_wlt_wo->transfer(destinations, 0, 0, TX_DEFAULT_FEE, {}, {}, stub_tx);
    alice_wlt->sign_transfer_files("virie_tx_unsigned", "virie_tx_signed", stub_tx);
    alice_wlt_wo->submit_transfer_files("virie_tx_signed", stub_tx);
  }

  {
    /// balances after transfer is sent from alice_wo but before it is unlocked;
    /// alice and alice_wo balances are equal
    alice_balance.unlocked = 0;
    alice_balance.total -= send_amount + TX_DEFAULT_FEE;
    alice_balance.out = send_amount;
    alice_wo_balance = alice_balance;

    bob_balance.total += send_amount;
    bob_balance.in = send_amount;
  }

  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
  check_balance("bob", bob_wlt, bob_balance);
  mine();
  {
    /// balances after sending transfer from alice_wo and unlocking it;
    /// alice and alice_wo balances are equal
    alice_balance.unlocked = alice_balance.total;
    alice_balance.out = 0;
    alice_wo_balance = alice_balance;

    bob_balance.unlocked += send_amount;
    bob_balance.in = 0;
  }
  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
...

  {
    ...
    alice_wlt_wo->transfer(destinations, 0, 0, TX_DEFAULT_FEE, {}, {}, stub_tx);
    alice_wlt->sign_transfer_files("virie_tx_unsigned", "virie_tx_signed", stub_tx);
    alice_wlt->transfer(destinations, 0, 0, TX_DEFAULT_FEE, {}, {}, stub_tx);
  }

  {
    /// balances after transfer is sent from alice but before it is unlocked;
    /// alice and alice_wo balances are equal because alice_wo wallet have key image of out that used in this transaction
    /// (from previous transactions out)
    alice_balance.unlocked = 0;
    alice_balance.total -= send_amount + TX_DEFAULT_FEE;
    alice_balance.out = send_amount;
    alice_wo_balance = alice_balance;
  ...
  mine();
  {
    /// balances after sending transfer from alice and unlocking it;
    /// alice and alice_wo balances are equal because alice_wo wallet have key image of out that used in this transaction
    /// (from previous transactions out)
    alice_balance.unlocked = alice_balance.total;
    alice_balance.out = 0;
  ...
  CHECK_AND_ASSERT(r, false);

  miner_wlt->refresh();
  miner_wlt->transfer(1 * COIN, alice_wlt->get_account().get_public_address());
  {
    /// alice balance after sending transaction from bob - equal to alice_wo balance
    alice_balance.in = 1 * COIN;
    alice_balance.unlocked = alice_balance.total;
    alice_balance.total = alice_balance.in;
    alice_wo_balance = alice_balance;
  }
  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
  ...
  {
    /// balances after sending transfer from alice and unlocking it;
    /// alice and alice_wo balances are not equal because alice_wo wallet does not have key image of out that used in
    /// this transaction
    alice_balance.unlocked = 0;
    alice_balance.total -= send_amount + TX_DEFAULT_FEE;
    alice_balance.out = send_amount;
    alice_wo_balance = alice_balance;
    alice_wo_balance.out = 0;
  ...
  {
    /// balances after sending transfer from alice and unlocking it;
    /// alice and alice_wo balances are equal because alice_wo wallet also sending transaction
    alice_balance.unlocked = alice_balance.total;
    alice_balance.out = 0;
    alice_wo_balance = alice_balance;
  ...
  {
    /// alice and alice_wo balances are not equal because alice_wo's transfer was rejected and alice_wo wallet does
    /// not have private spent key to determine its outgoing transactions
    alice_wo_balance.total = 1 * COIN;
    alice_wo_balance.unlocked = alice_wo_balance.total;
  }
  check_balance("alice", alice_wlt, alice_balance);
  check_balance("alice_wo", alice_wlt_wo, alice_wo_balance);
