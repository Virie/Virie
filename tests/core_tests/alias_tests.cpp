...
bool put_alias_via_tx_to_list(std::vector<test_event_entry>& events,
  ...
  uint64_t reward = currency::get_alias_cost_from_fee(ai2.m_alias, fee_median);
...
bool gen_alias_tests::check_too_many_aliases_registration(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  uint64_t estimated_alias_cost = get_alias_cost_from_fee(
    std::string(ALIAS_MINIMUM_PUBLIC_SHORT_NAME_ALLOWED, 'a'),
    c.get_blockchain_storage().get_tx_fee_median());
...
bool gen_alias_strange_data::generate(std::vector<test_event_entry>& events) const
{
  ...
  REWIND_BLOCKS_N(events, blk_middle, blk_0, miner_acc, 2);
  REWIND_BLOCKS(events, blk_0r, blk_middle, miner_acc);
...
bool gen_alias_reg_with_locked_money::generate(std::vector<test_event_entry>& events) const
{
  ...
  destinations.push_back(tx_destination_entry(get_alias_cost_from_fee(ai.m_alias, TESTS_DEFAULT_FEE), null_addr));
...
bool gen_alias_too_small_reward::generate(std::vector<test_event_entry>& events) const
{
  ...
  const alias_entry aliases[] = {
    {"a",          pub_addr_from_string("HaNanRkC4Lqh8v32h7PNBBbHzUq5xCMgk6TkEHvF5ho1XruDmSxjBdegLS7Eo1EQqCNKKgwJTS5XsCxRNs967DzcFquMNB5")},
    {"bb",         pub_addr_from_string("HbUncz4isbWT6e6rKbuZPn8ECWdfxoZxwhQXeaAXytVrQtGADah5La1MhyVJPc2hFvXdt4WjAS7rMD6xjjLFtL73BHM1V5Z")},
    {"ccc",        pub_addr_from_string("HdgyJCHxrawN1cucXdw7Da4F9XeW4LZesRANMbrVLKntb9H3M6bTCrR1zD7dYdt7RwHztxbJ8Dofw1wSm417aznNSoERYBc")},
    {"dddd",       pub_addr_from_string("HdUzDd5HR4c7i3BJQAVZjFcUAxpup8k1W9KKD6k5MQKmbu429dyQC3iWmHx5KGgmF5dbAs2DnbYmhCZGUNJK5mVVS4P9Mcm")},
    {"eeeee",      pub_addr_from_string("HeJAiCjZ5CdWxmJsVQtrHLasqntPq43ArCSJBUanDzpQEsHkjTS1W3HYjFvzNzFGeC5QwP88cuQM9SoQayib8Rd2TkaKqh8")},
    {"ffffff",     pub_addr_from_string("HeqJJNAsXCNTQFNQfANzkAK9YaZ6FTz49TuDyPyzk41pWgaoAudcPVrYNMa3qDetFPDHDnhbY5Hn3b37efDa4PU669EH8DX")},
    {"ggggggg",    pub_addr_from_string("HfAc4XA8KTuZLFdaEhGH6UR6iXFVPFxSJYH5AwwyzzKidMSjyW2XCkLNDmFT6vj6hd3WMiJs5P552EkipeeXsBj2KSyQZVE")},
    {"hhhhhhhh",   pub_addr_from_string("Hfe6hJ8DvwwBRxEYq9oizPTQ2v7gC2A7mC3MnxzSeBzKcr3F3tfE89c2HidxFrrkfKSEweVVx6cYGDbdZkyoWJSXPQxHseB")},
    {"iiiiiiiii",  pub_addr_from_string("HfkaeuiWFKb5UQVPtUNM6L65PwcFMLhBCNkHfg4kxHMdKX1SpHVrXfRegH6XBiqhvZawntWYdKRPuLkFbJtZmYki9eWu2ht")},
    {"jjjjjjjjjj", pub_addr_from_string("Hg63TMEchFN2uKzrhER13GapemB4tFPirZsiJ6kijaPBan9ytHbPJZeEK1m7Lig5jhjDubptpMiuX7bxHwLbQTwh1eEGqXX")}
  };
  ...
  transaction tx_1 = AUTO_VAL_INIT(tx_1);
  const bool r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), miner_acc.get_public_address(), 3 * aliases_count * TESTS_DEFAULT_FEE * 10, 3 * aliases_count, TESTS_DEFAULT_FEE, tx_1);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  ...
  for (const auto& ae : aliases)
  {
    const uint64_t alias_reward = get_alias_cost_from_fee(ae.name, TESTS_DEFAULT_FEE);

    transaction tx = AUTO_VAL_INIT(tx);
    DO_CALLBACK(events, "mark_invalid_tx"); // should be rejected, because it's paid TESTS_MINIMUM_FEE / 100; divide == 100 because TESTS_MINIMUM_FEE has been increased 10 times, but ALIAS_VERY_INITAL_COST stayed the same
    if (!make_tx_reg_alias(events, generator, blk_1, ae.name, ae.addr, TESTS_MINIMUM_FEE / 100, miner_acc, tx, used_sources))
      return false;
...
//     if (!make_tx_reg_alias(events, generator, blk_1, ae.name, ae.addr, alias_reward - TX_DEFAULT_FEE / 10, miner_acc, tx, used_sources))
//       return false;

    // should be accepted
    if (!make_tx_reg_alias(events, generator, blk_1, ae.name, ae.addr, alias_reward, miner_acc, tx, used_sources))
      return false;
  ...
  for (const auto& ae : aliases)
  {
    DO_CALLBACK_PARAMS(events, "check_alias", ae);
  }
...
bool gen_alias_too_many_regs_in_block_template::generate(std::vector<test_event_entry>& events) const
{
  ...
  m_estimated_alias_cost = currency::get_alias_cost_from_fee(
    std::string(ALIAS_MINIMUM_PUBLIC_SHORT_NAME_ALLOWED, 'a'), fee_median);
