...
{
  BLOCK_VALIDATION_INIT_GENERATE();
  REWIND_BLOCKS_N(events, blk_0r, blk_0, miner_account, BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
...
gen_blocks_timestamps_median::gen_blocks_timestamps_median()
{
  REGISTER_CALLBACK("check_blocks_timestamps", gen_blocks_timestamps_median::check_blocks_timestamps);
}

bool gen_blocks_timestamps_median::check_blocks_timestamps(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  if(c.get_blockchain_storage().get_last_timestamps_check_window_median() != current_timestamps.at((ev_index / 2) - 1))
  {
    return false;
  }
  return true;
...
bool gen_blocks_timestamps_median::generate(std::vector<test_event_entry>& events)
{
  BLOCK_VALIDATION_INIT_GENERATE();
  block blk_new, blk_old;
  blk_old = blk_0;
  blk_new = blk_0;
  std::vector<uint64_t> timestamps;
  uint64_t timestamp = blk_0.timestamp;

  for(int i = 1; i < BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW+1; i++)
  {
    blk_old = blk_new;
    blk_new = AUTO_VAL_INIT(blk_new);
    timestamp += 100;
    generator.construct_block_manually(blk_new, blk_old, miner_account, test_generator::bf_timestamp, 0, 0, timestamp);
    timestamps.push_back(timestamp);
    current_timestamps.push_back(epee::misc_utils::median(timestamps));

    events.push_back(blk_new);
    DO_CALLBACK(events, "check_blocks_timestamps");
  }
  return true;
}
