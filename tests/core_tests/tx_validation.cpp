...
bool gen_tx_extra_double_entry::generate(std::vector<test_event_entry>& events) const
{
  ...
  GENERATE_ACCOUNT(miner_account);
  MAKE_GENESIS_BLOCK(events, blk_0, miner_account, ts_start);
  REWIND_BLOCKS_N(events, blk_middle, blk_0, miner_account, 2);
  REWIND_BLOCKS(events, blk_0r, blk_middle, miner_account);
  ...
  uint64_t alias_reward = currency::get_alias_cost_from_fee(ai.m_alias, TESTS_DEFAULT_FEE);
  sources.clear();
  destinations.clear();
  fill_tx_sources_and_destinations(events, blk_0r, miner_account, miner_account, MK_TEST_COINS(1), TESTS_DEFAULT_FEE + alias_reward, 0, sources, destinations);
