...
bool wallet_test_core_proxy::add_genesis_block(const currency::block &b, test_generator &generator)
{
  if (!m_blocks.empty())
    return true;
  generator.get_block_chain(m_blocks, currency::get_block_hash(b), 1);
  generator.build_outputs_indext_for_chain(m_blocks, m_oi, m_txs_outs);
  return true;
}
...
bool wallet_test_core_proxy::get_transfer_address(const std::string& adr_str, currency::account_public_address& addr, currency::bc_payment_id& payment_id)
{
