...
bool escrow_incorrect_proposal::check_normal_proposal(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  bc_services::contract_private_details cpd = contracts.begin()->second.private_details;
...
bool escrow_incorrect_proposal::check_incorrect_proposal(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  if (contracts.size() != 0)
  {
    ...
    for (auto &c : contracts)
      s += "    " + std::to_string(i) + ") " + c.second.private_details.comment + "\r\n", ++i;
...
bool escrow_incorrect_proposal_acceptance::check_normal_acceptance(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_AND_ASSERT_MES(contracts.begin()->second.private_details == m_cpd, false, "Alice has invalid contract's private details");
  ...
  CHECK_AND_ASSERT_MES(contracts.begin()->second.private_details == m_cpd, false, "Bob has invalid contract's private details");
...
bool escrow_custom_test::generate(std::vector<test_event_entry>& events) const
{
  ...
  const uint64_t chunk_amount = TX_DEFAULT_FEE;
  const size_t chunks_count = 20;
  const uint64_t alice_bob_start_amount = chunks_count * chunk_amount;
  ...
  // setup custom tests by filling in callback details
  {
    ...
    cd.cpd.amount_a_pledge = 70 * chunk_amount / 10;
    cd.cpd.amount_b_pledge = 94 * chunk_amount / 10;
    cd.cpd.amount_to_pay   = 18 * chunk_amount / 10;
    ...
    cd.a_proposal_fee      = 1 * TX_DEFAULT_FEE;
    cd.b_accept_fee        = 1 * TX_DEFAULT_FEE;
    cd.b_release_fee       = 1 * TX_DEFAULT_FEE;
...

  {
    ...
    cd.unlock_time         = 0;
    cd.proposal_expiration_period = 3600;
    cd.a_proposal_fee      = TX_DEFAULT_FEE;
    cd.b_accept_fee        = TX_DEFAULT_FEE;
    cd.b_release_fee       = TX_DEFAULT_FEE;
    cd.payment_id          = "payment id";
    cd.release_type        = BC_ESCROW_SERVICE_INSTRUCTION_RELEASE_NORMAL;
    test_details.push_back(cd);
  }
...
bool escrow_custom_test::do_custom_test(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events, const escrow_custom_test_callback_details& cd)
{
  ...
  CHECK_AND_ASSERT_MES(contracts.begin()->second.payment_id.id == cd.payment_id, false, "incorrect payment_id was found in Bob's contract: " << contracts.begin()->second.payment_id.id << ", expected: " << cd.payment_id);
  crypto::hash ms_id = contracts.begin()->first;
  ...
  CHECK_AND_ASSERT_MES(contracts.begin()->second.payment_id.id == cd.payment_id, false, "incorrect payment_id was found in Alice's contract: " << contracts.begin()->second.payment_id.id << ", expected: " << cd.payment_id);
...
bool escrow_incorrect_cancel_proposal::generate(std::vector<test_event_entry>& events) const
{
  ...
  const uint64_t chunk_amount = 2 * TX_DEFAULT_FEE;
  const size_t chunks_count = 20;
  ...
  // prepare contract details
  m_cpd.amount_a_pledge = chunk_amount * 12 / 10;
  m_cpd.amount_b_pledge = chunk_amount * 13 / 10;
  m_cpd.amount_to_pay = chunk_amount * 3 / 10;
  m_cpd.a_addr = alice_acc.get_public_address();
  m_cpd.b_addr = bob_acc.get_public_address();
  m_bob_fee_release = chunk_amount; // Alice states that Bob should pay this much money for upcoming contract release (which will be sent by Alice)
...
// Test idea: 
  ...
  transaction tx_0 = AUTO_VAL_INIT(tx_0);
  bool r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), alice_acc.get_public_address(), MK_TEST_COINS(20), 20, TX_DEFAULT_FEE, tx_0);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_0);

  transaction tx_1 = AUTO_VAL_INIT(tx_1);
  r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), bob_acc.get_public_address(), MK_TEST_COINS(20), 20, TX_DEFAULT_FEE, tx_1);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_1);
...
bool escrow_cancellation_and_tx_order::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  std::shared_ptr<tools::wallet2> alice_wlt = init_playtime_test_wallet(events, c, m_accounts[ALICE_ACC_IDX]);
  alice_wlt->refresh();
  CHECK_AND_ASSERT_MES(check_balance_via_wallet(*alice_wlt.get(), "Alice", MK_TEST_COINS(20), 0, MK_TEST_COINS(20), 0, 0), false, "");

  std::shared_ptr<tools::wallet2> bob_wlt = init_playtime_test_wallet(events, c, m_accounts[BOB_ACC_IDX]);
  bob_wlt->refresh();
  CHECK_AND_ASSERT_MES(check_balance_via_wallet(*bob_wlt.get(), "Bob", MK_TEST_COINS(20), 0, MK_TEST_COINS(20), 0, 0), false, "");
  ...
  CHECK_AND_ASSERT_MES(r, false, "get_contracts() for Alice failed");
  CHECK_AND_ASSERT_MES(contracts.size() == 1, false, "Alice has incorrect number of contracts: " << contracts.size());
  CHECK_AND_ASSERT_MES(contracts.begin()->second.private_details == cpd, false, "Alice has invalid contract's private details");
  ...
  CHECK_AND_ASSERT_MES(r, false, "get_contracts() for Bob failed");
  CHECK_AND_ASSERT_MES(contracts.size() == 1, false, "Bob has incorrect number of contracts: " << contracts.size());
  CHECK_AND_ASSERT_MES(contracts.begin()->second.private_details == cpd, false, "Bob has invalid contract's private details");
...
bool escrow_cancellation_proposal_expiration::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  uint64_t alice_start_balance = MK_TEST_COINS(20);
  std::shared_ptr<tools::wallet2> alice_wlt = init_playtime_test_wallet(events, c, m_accounts[ALICE_ACC_IDX]);
  ...
  uint64_t bob_start_balance = MK_TEST_COINS(20);
  std::shared_ptr<tools::wallet2> bob_wlt = init_playtime_test_wallet(events, c, m_accounts[BOB_ACC_IDX]);
...
bool escrow_cancellation_acceptance_expiration::generate(std::vector<test_event_entry>& events) const
{
  ...
  transaction tx_0 = AUTO_VAL_INIT(tx_0);
  bool r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), alice_acc.get_public_address(), MK_TEST_COINS(10), 20, TX_DEFAULT_FEE, tx_0);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_0);

  transaction tx_1 = AUTO_VAL_INIT(tx_1);
  r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), bob_acc.get_public_address(), MK_TEST_COINS(10), 20, TX_DEFAULT_FEE, tx_1);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_1);
...
bool escrow_cancellation_acceptance_expiration::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  uint64_t alice_start_balance = MK_TEST_COINS(10);
  std::shared_ptr<tools::wallet2> alice_wlt = init_playtime_test_wallet(events, c, m_accounts[ALICE_ACC_IDX]);
  CHECK_AND_ASSERT_MES(refresh_wallet_and_check_balance("", "Alice", alice_wlt, alice_start_balance, true, UINT64_MAX, alice_start_balance), false, "");

  uint64_t bob_start_balance = MK_TEST_COINS(10);
  std::shared_ptr<tools::wallet2> bob_wlt = init_playtime_test_wallet(events, c, m_accounts[BOB_ACC_IDX]);
  CHECK_AND_ASSERT_MES(refresh_wallet_and_check_balance("", "Bob", bob_wlt, bob_start_balance, true, UINT64_MAX, bob_start_balance), false, "");
...
bool escrow_zero_amounts::generate(std::vector<test_event_entry>& events) const
{
  ...
  transaction tx_0 = AUTO_VAL_INIT(tx_0);
  bool r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), alice_acc.get_public_address(), MK_TEST_COINS(20), 20, TESTS_DEFAULT_FEE, tx_0);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_0);

  transaction tx_1 = AUTO_VAL_INIT(tx_1);
  r = construct_tx_with_many_outputs(events, blk_0r, miner_acc.get_keys(), bob_acc.get_public_address(), MK_TEST_COINS(20), 20, TESTS_DEFAULT_FEE, tx_1);
  CHECK_AND_ASSERT_MES(r, false, "construct_tx_with_many_outputs failed");
  events.push_back(tx_1);
...
bool escrow_zero_amounts::c1(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events)
{
  ...
  CHECK_AND_ASSERT_MES(refresh_wallet_and_check_balance("", "Alice", alice_wlt, MK_TEST_COINS(20), true, UINT64_MAX, MK_TEST_COINS(20)), false, "");
  CHECK_AND_ASSERT_MES(refresh_wallet_and_check_balance("", "Bob", bob_wlt, MK_TEST_COINS(20), true, UINT64_MAX, MK_TEST_COINS(20)), false, "");

  CHECK_AND_ASSERT_MES(c.get_pool_transactions_count() == 0, false, "Incorrect txs count in the pool");

  // a_pledge > 0, b_pledge + amount_to_pay = 0
  bc_services::contract_private_details cpd = AUTO_VAL_INIT(cpd);
  cpd.amount_a_pledge = MK_TEST_COINS(1);
