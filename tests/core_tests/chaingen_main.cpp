...
namespace
{
  ...
  const command_line::arg_descriptor<std::string> arg_filter_tests                = {"filter-tests", "", "", command_line::flag_t::not_use_default | command_line::flag_t::file_support };
  const command_line::arg_descriptor<bool>        arg_enable_debug_asserts        = {"enable-debug-asserts", "" };
  const command_line::arg_descriptor<bool>        arg_list_tests                  = {"list-tests", "List of tests" };
...
{                                                                                                          \
  autodoc_tests.push_back(#genclass);                                                                      \
  if(!autodoc && test_filter.can_run(#genclass))                                \
  {                                                                                                        \
...
#define CALL_TEST(test_name, function)                                                                     \
  {                                                                                                        \
    std::cout << concolor::normal << "#TEST# " << test_name << " " << "start..." << std::endl;             \
    if(!function())                                                                                        \
    {                                                                                                      \
  ...
  size_t event_index_from, size_t event_index_to, bool deinit_core, cb_1_t after_init_cb, cb_2_t before_deinit_cb)
{
  ...
  crc.tx_pool_min_fee = [](uint64_t)->uint64_t {return TESTS_MINIMUM_FEE;};
...
{
public:
  void add_filters(const std::string &cfilter_str)
  {
    if (cfilter_str.empty())
      return;
    std::string filter_str = cfilter_str;
    boost::algorithm::trim(filter_str);
    std::vector<std::string> filters;
    boost::algorithm::split(filters, filter_str, boost::is_any_of(",\n"));
    for (auto &filter : filters)
    {
      boost::algorithm::trim(filter);
      if (filter.empty())
        continue;
      auto it = std::find(filter.begin(), filter.end(), '*');
      while (it != filter.end())
      {
        filter.insert(it - filter.begin(), ".");
        ++it;
        it = std::find(++it, filter.end(), '*');
      }
      m_filters.push_back(std::move(filter));
    }
  }

  bool can_run(const std::string &class_name) const
  {
    if (m_filters.empty())
      return true;
    for (auto &filter : m_filters)
      if (boost::regex_match(class_name, boost::regex(filter)))
        return true;
    return false;
  }
...
int main(int argc, char* argv[])
{
  ...
  command_line::add_arg(desc_options, arg_filter_tests);
  ...
  else
  {
    ...
    const bool autodoc = command_line::get_arg(g_vm, arg_list_tests);

    test_filter test_filter;
    if (command_line::has_arg(g_vm, arg_filter_tests))
      test_filter.add_filters(command_line::get_arg(g_vm, arg_filter_tests));
    
    if (!autodoc && test_filter.empty())
    {
    ...
    std::list<std::string> autodoc_tests;
    ...
    GENERATE_AND_PLAY(offer_with_damaged_body);
    ...
    GENERATE_AND_PLAY(gen_invalid_checkpoints);
    ...
    GENERATE_AND_PLAY(wallet_watch_only);
    ...
    GENERATE_AND_PLAY(pos_the_same_kernels_in_different_blocks);
    ...
    GENERATE_AND_PLAY(gen_difficulty_caching);
    ...
    GENERATE_AND_PLAY(gen_blocks_timestamps_median);
...
#undef MARK_TEST_AS_POSTPONED
    ...
    if (autodoc)
    {
      ...
      for (const auto & t : autodoc_tests)
        std::cout << '\t' << t << '\n';
      std::cout << std::flush;
