...
inline bool refresh_wallet_and_check_contract_state(const char* wallet_name, std::shared_ptr<tools::wallet2> wallet, uint32_t expected_contract_state, crypto::hash contract_id, size_t block_to_be_fetched = SIZE_MAX)
{
  ...
  tools::wallet2::escrow_contracts_container contracts;
  wallet->get_contracts(contracts);
  ...
  std::vector<tx_source_entry>& used_sources /* IN/OUT */)
{
  ...
  bc_services::pack_attachment_as_json(cpd, sa);
  sa.flags |= TX_SERVICE_ATTACHMENT_DEFLATE_BODY;
  ...
  bc_services::proposal_body* p_pb = nullptr /* OUT */ )
{
  ...
  bc_services::pack_attachment_as_bin(*p_pb, sa);
  sa.flags |= TX_SERVICE_ATTACHMENT_DEFLATE_BODY;
...
)
{
  ...
  bc_services::pack_attachment_as_bin(rtb, tsa);
  tsa.flags |= TX_SERVICE_ATTACHMENT_DEFLATE_BODY;
...
)
{
  ...
  bc_services::pack_attachment_as_bin(ctb, tsa);
  tsa.flags |= TX_SERVICE_ATTACHMENT_DEFLATE_BODY;
...
inline bool check_contract_state(const tools::wallet2::escrow_contracts_container& contracts, const bc_services::contract_private_details& cpd, tools::wallet_rpc::escrow_contract_details::contract_state state, const char* party)
{
  ...
  CHECK_AND_ASSERT_MES(contracts.begin()->second.private_details == cpd, false, party << " has invalid contract's private details");
