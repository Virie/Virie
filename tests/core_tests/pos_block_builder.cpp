  ...
  keypair tx_one_time_key /*= keypair::generate()*/)
{
  ...
  if (!alias.m_alias.empty())
    alias_reward = currency::get_alias_cost_from_fee(alias.m_alias, TESTS_DEFAULT_FEE);
  ...
  std::function<bool(currency::block&)> before_sign_cb, currency::block& output)
{
  ...
  currency::wide_difficulty_type difficulty = c.get_blockchain_storage().get_cached_next_difficulty(true);
