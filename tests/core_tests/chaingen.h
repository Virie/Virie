...
#define TESTS_MINIMUM_FEE                   ((uint64_t)TX_MINIMUM_FEE)
...
namespace concolor
{
  ...
  {
    epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_white, true);
    return ostr;
  }
  ...
  {
    epee::colored_cout::cu_cout.set_color(epee::colored_cout::console_colors::console_color_green, true);
    return ostr;
  }
...
public:
  ...
  uint64_t get_last_n_blocks_fee_median(const crypto::hash& head_block_hash, size_t n = ALIAS_COST_PERIOD);
  ...
  bool init_test_wallet(const currency::account_base& account, const currency::block& genesis_block, std::shared_ptr<tools::wallet2> &result);
...
#define CREATE_TEST_WALLET(WLT_VAR, ACCOUNT, GENESIS_BLOCK) \
  ...
  generator.init_test_wallet(ACCOUNT, GENESIS_BLOCK, WLT_VAR)
