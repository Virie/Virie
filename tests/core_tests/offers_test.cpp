...
offer_with_damaged_body::offer_with_damaged_body()
{
  REGISTER_CALLBACK_METHOD(offer_with_damaged_body, check);
}

bool offer_with_damaged_body::generate(std::vector<test_event_entry>& events)
{
  uint64_t ts_start = 1338224400;
  GENERATE_ACCOUNT(miner_account);
  MAKE_GENESIS_BLOCK(events, blk_0, miner_account, ts_start);
  ...
  MAKE_TX_LIST_START_WITH_ATTACHS(events, txs_blk, miner_account, miner_account, MK_TEST_COINS(1), blk_11, attachments)
  MAKE_NEXT_BLOCK_TX_LIST(events, blk_12, blk_11, miner_account, txs_blk)
  m_tx_hashes_with_correct_order.push_back(blk_12.tx_hashes[0]);

  transaction tx = AUTO_VAL_INIT(tx);
  std::vector<tx_destination_entry> destinations;
  std::vector<tx_source_entry> sources;
  bool r = fill_tx_sources(sources, events, blk_12, miner_account.get_keys(), TESTS_DEFAULT_FEE, 0);
  CHECK_AND_ASSERT_MES(r, false, "fill_tx_sources failed");
//  r = construct_tx(miner_account.get_keys(), sources, destinations, attachments, tx, 0);
//  CHECK_AND_ASSERT_MES(r, false, "construct_tx failed");

//  auto &a = boost::get<currency::tx_service_attachment>(tx.attachment[0]);
//  a.body += "damage";
//  tx.extra.pop_back();
//  currency::add_attachments_info_to_extra(tx.extra, tx.attachment);

  auto sender_account_keys = miner_account.get_keys();
  auto crypt_destination_addr = get_crypt_address_from_destinations(sender_account_keys, destinations);
  crypto::secret_key one_time_secret_key;
  auto extra = std::vector<extra_v>();
  ...
  auto &a = boost::get<tx_service_attachment>(tx.attachment[0]);
  a.body = "damage" + a.body;
  ...
  BOOST_FOREACH(const tx_destination_entry& dst_entr, shuffled_dsts)
  {
    ...
    r = construct_tx_out(dst_entr, txkey.sec, output_index, tx, deriv_cache);
  ...
  events.push_back(tx);
  MAKE_NEXT_BLOCK_TX1(events, blk_13, blk_12, miner_account, tx)

  MAKE_TX_LIST_START_WITH_ATTACHS(events, tx_14, miner_account, miner_account, MK_TEST_COINS(1), blk_13, attachments)
  MAKE_NEXT_BLOCK_TX_LIST(events, blk_14, blk_13, miner_account, tx_14)

  m_tx_hashes_with_correct_order.push_back(blk_14.tx_hashes[0]);
...
bool offer_with_damaged_body::check(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  std::list<bc_services::offer_details_ex> offers;

  get_all_offers(c, offers);
  CHECK_EQ(2, offers.size());

  CHECK_EQ(offers.front().tx_hash, m_tx_hashes_with_correct_order[0]);
  CHECK_EQ(offers.back().tx_hash, m_tx_hashes_with_correct_order[1]);
