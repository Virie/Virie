...
struct gen_block_ts_in_past : public gen_block_verification_base<BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW + 1>
{
  bool generate(std::vector<test_event_entry>& events) const;
};
...
struct gen_blocks_timestamps_median : public test_chain_unit_base
{
  gen_blocks_timestamps_median();
  bool check_blocks_timestamps(currency::core& c, size_t ev_index, const std::vector<test_event_entry>& events);
