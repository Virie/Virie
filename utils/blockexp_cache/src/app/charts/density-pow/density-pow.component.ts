...
@Component({
  selector: 'app-density-pow',
  templateUrl: './density-pow.component.html',
  styleUrls: ['./density-pow.component.scss']
})
export class DensityPowComponent implements OnInit, OnDestroy {
  navIsOpen: boolean;
  ...
  initialChart() {
    ...
    this.chartSubscription = this.httpService.getChart(this.activeChart, this.startDate, this.endDate).subscribe(data => {
      ...
      for (let i = 1; i < this.InputArray.length; i++) {
        densityPow.push([this.InputArray[i].timestamp * 1000, this.InputArray[i].blocksCount]);
      }
      this.densityPow = DensityPowComponent.drawChart(
        false,
        '',
        'Blocks',
        this.seriesData = [
          {type: 'area', name: 'Blocks', data: densityPow}
        ]
      );
