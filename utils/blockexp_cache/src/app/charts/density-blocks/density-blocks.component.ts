...
@Component({
  selector: 'app-density-blocks',
  templateUrl: './density-blocks.component.html',
  styleUrls: ['./density-blocks.component.scss']
})
export class DensityBlocksComponent implements OnInit, OnDestroy {
  navIsOpen: boolean;
  ...
  constructor(private httpService: HttpService, private mobileNavState: MobileNavState) {
    ...
    this.activeChart = 'densityBlocks';
    this.btnActive = 'week';
  ...
  initialChart() {
    ...
    this.chartSubscription = this.httpService.getChart(this.activeChart, this.startDate, this.endDate).subscribe(data => {
      ...
      for (let i = 1; i < this.InputArray.length; i++) {
        densityBlocks.push([this.InputArray[i].timestamp * 1000, this.InputArray[i].blocksCount]);
      }
      this.densityBlocks = DensityBlocksComponent.drawChart(
        false,
        '',
        'Blocks',
        this.seriesData = [
          {type: 'area', name: 'Blocks', data: densityBlocks}
        ]
      );
